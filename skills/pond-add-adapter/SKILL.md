...
| 11 | Windows | Where the agent writes on Windows, env overrides, encoding quirks (CRLF, UTF-16, path separators). | `build-and-test-windows` CI runs the full test suite natively on every main push and on same-repo PRs labelled `windows` - label an adapter PR; a fork PR gets it when a maintainer pushes the branch or on merge |
...
`cargo fmt --check`, `cargo clippy --all-targets -- -D warnings`, `cargo test` - all green locally (`--all-targets` matters: Linux CI lints only the library, the Windows leg lints the tests too). CI additionally runs the identical test suite natively on Windows (`build-and-test-windows`) for same-repo PRs labelled `windows` - add the label to an adapter PR - which is what makes decision-table row 11 checked instead of aspirational; a fork PR gets that leg when a maintainer pushes the branch or on merge. That is the whole bar; no benchmark run is part of an adapter PR.
