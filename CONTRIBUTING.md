...
Issues and pull requests are welcome. For anything larger than a bug fix, comment on the matching [roadmap](README.md#roadmap) issue first - or open one - so we agree on scope before you invest. Repo conventions live in [AGENTS.md](AGENTS.md); the system contract is [docs/spec.md](docs/spec.md). Security issues go through [SECURITY.md](.github/SECURITY.md).
...
- `cargo fmt --check`, `cargo clippy -- -D warnings`, `cargo test` green is the whole bar. No benchmarks: adapters are structurally isolated from the store, and a guard test enforces it.

## Everything else

Run the same three commands before pushing. CI re-runs them on Linux and Windows.
