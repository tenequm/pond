...
STORE_URL="${STORE_URL:-$(awk -F'"' '/^\[storage\]/{s=1;next} /^\[/{s=0} s&&/^path/{print $2;exit}' ~/.config/pond/config.toml)}"
...
probe search          "$TMP/search.txt"  $POND search "read performance optimization lance" --limit 10
...
iops() { awk -v c="$1" '$1 == c {print $2; exit}' "$TMP/serve.txt"; }
ms() { grep -F "$1" "$TMP/ops.txt" | awk '{print int($(NF-1))}'; }
printf '{"date":"%s","commit":"%s","get_session_sid_s":%s,"get_session_mid_s":%s,"get_message_s":%s,"search_s":%s,"sql_count_s":%s,"equivalence":"OK","fts_iops":%s,"vector_iops":%s,"get_message_iops":%s,"search_iops":%s,"open_store_ms":%s,"row_counts_ms":%s,"oracle_warm_ms":%s}\n' \
...
for k, v in cur.items():
    if k in ("date", "commit", "equivalence"):
        continue
    p = prev.get(k)
    d = f"{(v - p) / p * 100:+.0f}%" if p else "n/a"
    print(f"{k:<20}{p if p is not None else '-':>12}{v:>12}{d:>10}")
EOF
