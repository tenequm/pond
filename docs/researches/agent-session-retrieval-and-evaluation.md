...
- Toward Conversational Agents with Context- and Time-Sensitive Long-term Memory (arXiv:2406.00057). Pure semantic + BM25 retrieval fails on temporal and metadata queries ("what did I try last Tuesday"). pond's `prefilter-pushdown` plus a timestamp scalar index already addresses this - a seam to keep, not add.
