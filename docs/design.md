...
10. **Data is derivable from sources.** Every byte in pond's datasets is reproducible by re-running the registered adapters against the same source data. pond is a derived index, not a system of record. This is what lets the recovery path be `rm -rf $POND_DATA_DIR && pond sync` rather than a dedicated rollback toolkit (see 3.6 "Recovery model"). The corollary is that source data is the operator's responsibility to preserve; pond does not back up the original JSONL / archive files it ingests from.
...
Concat policy changes require re-ingest; re-ingest is idempotent per invariant 2 (matching PKs with new content overwrite via `merge_insert`). Embedding rebuilds follow naturally because the embedded `search_text` changes.
...
**Recovery model.** pond's data is derivable from the registered adapters' source data (invariant 10 in 2.3): every byte in `sessions.lance` / `messages.lance` / `parts.lance` / `embeddings.lance` is reproducible by re-running the adapter against the same source bytes. The recovery path for any corruption - bad adapter shipment, regretted ingest, filesystem-level damage - is `rm -rf $POND_DATA_DIR && pond sync`. Re-ingest is idempotent on the canonical PKs (invariants 1 + 2), so re-running against unchanged source data converges to the same canonical state. For the rare case where source data is no longer reachable (sources deleted, sessions expired) the operator's defense is `pond export` ahead of risky operations, plus Lance's manifest retention (3.2.0 `auto_cleanup`) as a defense-in-depth fallback inspectable via `lance-cli` directly. pond does not ship `versions list` / `checkout` / `restore` CLI verbs in v1; the time-travel-as-feature use case (point-in-time queries, audit endpoints) is deferred to section 4.
