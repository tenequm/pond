...
11. **Namespace resolution lives in one function.** `handlers::resolve_namespace(namespace) -> Result<&str, ErrorEnvelope>` is the only code path that decides whether a request's namespace is acceptable. No wire handler hand-rolls the check. v1 returns `"local"`; multi-namespace pond will change this function's signature once to `(namespace, &Router) -> Result<&Arc<Store>>` and every call site picks up the right store without further edits.
...
13. **Prefilter pushdown is opt-in on every Lance `Scanner`.** Pond MUST call `Scanner::prefilter(true)` on every vector kNN and FTS query; the `lance` crate defaults it to `false` (only the `lancedb` wrapper - which pond does not depend on - defaults it to `true`, which is why the LanceDB docs describe prefilter as "the default"). Without it, Lance silently postfilters in memory and ignores the scalar indexes entirely (recall loss; fewer than `limit` results returned). Load-bearing: an integration test on real data MUST assert via `Scanner::explain_plan` that the scalar predicate appears as a `ScalarIndexQuery` / `ScalarIndexExec` node (prefilter) and not as a top-level `FilterExec` (postfilter).
14. **Sync is purely additive at the row level.** Pond is durable storage, not a source-derived cache. `merge_insert` uses `WhenMatched::DoNothing` across every table: incoming sync events fill gaps; matched rows are never overwritten, never tombstoned. Source data is therefore not authoritative against the stored row - it can be incomplete, rotated, or gone, and the stored row stays. Adapter output is still monotone over versions (a new adapter MUST produce a superset of the prior version's canonical rows for the same source); bug fixes that add rows just land as inserts on the next sync. Bug fixes that need to *change* an existing row's field values are a per-row migration, not a sync flag - deferred until the first concrete need lands. Bug fixes that need to *drop* previously-emitted rows are likewise migrations, not sync semantics. See `src/substrate.rs::merge_insert` for the enforcement point.
...
- One `lance::Session` is constructed at pond startup and shared across the four datasets. The Session carries the metadata + index caches and the `ObjectStoreRegistry` (the underlying object_store / S3 client). Sharing it means one cache pool covers all four tables and one S3 client serves all four datasets - load-bearing on object-store backends, where per-dataset Sessions would mean 4x connection pools and 4x credential refreshes. Routed through `DatasetBuilder::with_session` on open and `WriteParams.session` on write.
...
Interval default: 6h. Both operations logged at info level (versions removed, bytes reclaimed, fragments newly indexed, duration). Failures logged at warn and retried at the next interval; maintenance failures do not crash `pond serve`. `pond sync` always tail-calls the same logic after its `ensure_indices()` pass, so a CLI-only operator never has to remember a second verb; the call is a no-op when no rows were inserted. Both paths are safe against concurrent reads and writes (Lance OCC; Append-vs-Append commutes per `rust/lance/src/io/commit/conflict_resolver.rs:873-899`). Multiple pond processes running maintenance against the same dataset converge harmlessly. Configuration lives under `[maintenance]` in `config.toml` (`enabled`, `interval`, `retention`); the sync tail-call runs regardless of `enabled`.
...
Embedding granularity: one vector per message, no chunking. Measured against the real `~/.claude/projects/` corpus (689,702 messages), `search_text` is 20 tokens at the median and ~98% of messages are under 1024 tokens; chunking would multiply `embeddings` rows for the ~2% that exceed a chunk size only for RRF to dedup them straight back to one hit per message - work done purely to be undone. The embedding stage of `pond ingest` embeds each message's `search_text` whole, truncated to a `max_embed_tokens`-token prefix (default 1024): `max_embed_tokens` is passed as the `Qwen3TextEmbedding::from_hf` tokenizer `max_length`, so fastembed-rs truncates input past it before inference - pond owns no tokenizer of its own. The cap is a model-cost bound on the ~1% of messages past p99 (and the rare multi-100k-token outlier), not a retrieval-quality knob: the full uncapped `search_text` is still indexed by FTS (3.3), and the full message is always retrievable via `pond_get` (3.6.3). Truncation is deterministic - the same `search_text` at the same `max_embed_tokens` always yields the same capped input - keeping the embedding stable across retries and re-ingest a no-op. Because the cap selects which prefix is embedded, it is part of the vector's identity: `max_embed_tokens` is a PK column on `embeddings` (3.2.4), so changing the cap re-embeds the affected over-cap tail under a distinct row instead of silently overwriting (or, worse, silently keeping) a vector built under the old cap. Output dim is the model's fixed 1024: fastembed-rs exposes no Matryoshka dimension truncation, so the full hidden size is stored as-is. If a future need for variable dims arises, pond truncates and re-normalizes the vector itself (deferred, section 4).
...
- `ToolCallPart.*` - tool name, params, the lot. The previous policy indexed `name` plus string-leaf params for rare-token recall (file paths, CLI commands); in practice this surfaced naked tool-only assistant turns at the top of BM25 results and drowned out prose. Tool calls remain retrievable via `pond_get`.
...
**The unit of abort is the offending event, not the substream** (revised 2026-05-15). A violation surfaces as one `validation_failed` outcome (3.6.1) for the offending event and the substream continues from the next valid anchor. The rest of the session's events still land. Two consequences worth naming:
...
- **Live-write tools.** `pond_commit`, `pond_session_open` (and HTTP equivalents) for streaming events written as they arrive. Activation-time constraint (locked): streaming-event variants (Effect's `text-start`/`text-delta`/`text-end`, `reasoning-*`, `tool-params-*`) do not enter `parts.lance`; only the assembled final Part is persisted on turn completion. Preserves the append-only invariant and keeps OCC contention rare; aligns with pi-mono / Effect / opencode (all persist assembled state only).
...
- **Hosted-tier facade extensions.** Federated namespace credential vending; distributed read-through cache.
- **Per-namespace bucket separation.** Separate Lance dataset / bucket per tenant for KMS isolation when shared-prefix isolation is insufficient.
- **Nested namespaces.** Hard isolation between sub-spaces within a tenant (e.g. separate Lance dataset per project). v1 uses single namespace per tenant with `project` as a column.
