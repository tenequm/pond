...
- Lifecycle: start lazily (first tool call or `session_start`), never in the factory; stop in `session_shutdown`; health-check and backoff-respawn per `PondController`; missing binary produces a named fix ("pond not found - install: <one-liner>; then pond init"), never a raw ENOENT.
...
`--bootstrap` covers the zero-adapters cold start. The uncovered case: user has other adapters enabled (e.g. claude-code) but `pi-coding-agent` is not enabled - sync runs but captures no pi sessions. Flow on `session_start` when pond binary is present and `pond adapters list --format json` (verify exact flag against `pond adapters --help`) shows pi-coding-agent absent/disabled:
...
- `ctx.hasUI` false (json/print): skip the prompt, emit a single notify naming the command.
