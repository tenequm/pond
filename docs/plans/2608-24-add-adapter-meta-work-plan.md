...
Date: 2026-08-24. Owner: tenequm. Tracking: [#172](https://github.com/tenequm/pond/issues/172). Status: PR1 merged as `295c6cc` ([#173](https://github.com/tenequm/pond/pull/173), 2026-08-24); PR2 (letta-code, [#170](https://github.com/tenequm/pond/issues/170)) merged as `29aadc4` ([#177](https://github.com/tenequm/pond/pull/177), 2026-08-24); PR3 (grok-build, [#171](https://github.com/tenequm/pond/issues/171)) on `feat/grok-build-adapter` - spec doc, macOS+Windows sandbox fixture, adapter, conformance suite, and the deferred freshness-read-budget assertion (implemented as thread-local `peek_metering` in `jsonl.rs` + per-adapter unit tests rather than a harness field, since a process-global counter would cross-count under parallel `cargo test`; the budget is `min(file length, PEEK_TAIL_CAP)`, so a transcript under the cap pins the peek to one pass; back-applied to letta) all done, green, polished, and smoke-tested through the built binary (14 sessions, re-sync all fresh, default search hits); zero add-adapter skill edits were needed to ship it (the PR3 success bar), and both live-install gates passed - a real session in the real `~/.grok` on macOS and natively on Windows 11 x64, each through `pond adapters list` (probe detected), `adapters enable`, a `sync` with no `--path`, a fresh re-sync, and a default search. The restore face was then verified through the real CLI rather than the harness alone: `pond resume --to grok-build` served native fidelity, brought a subagent child and its parent-side sidecar with the parent, refused a second resume with exit 3, and reproduced both documented consequences of the out-of-footprint sidecar - and grok 1.0.5 itself resumed a restored directory that carried no `chat_history.jsonl`, quoted the restored history back, rebuilt the file, and appended to the stream. Six forward-looking learnings were then folded back into the skill in the same PR: the row 10 append-only probe (byte-prefix comparison across every mutating operation), the `peek_metering` read-budget test and its `min(len, cap)` trap, parent-side-only lineage and its additive-sync latch in row 7 (plus the row 9 out-of-footprint sidecar consequence), a no-history fallback for squashed upstream exports, the headless account sign-in gate at preflight, and two capture traps (Windows known-folder username leak, census arithmetic). Note: Phase A empirically overturned the deja-vu rewind claim - grok 1.0.x `updates.jsonl` is strictly append-only (`/rewind` appends a `rewind_marker`), so the tail-peek oracle stands. Targets: [#170](https://github.com/tenequm/pond/issues/170) (letta-code), [#171](https://github.com/tenequm/pond/issues/171) (grok-build), and every adapter after them.
