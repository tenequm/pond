...
>
> **Cost on Windows, measured (partial):** a bulk single-commit append copy is unchanged (63 ms -> 64 ms, 500 sessions / ~2500 rows, local NTFS, same machine). That is the case least able to show the cost, since the wrapper charges per *published object* and the manifest double-flush lives in the commit path - a per-commit sweep against the real corpus is the run that would settle it.
