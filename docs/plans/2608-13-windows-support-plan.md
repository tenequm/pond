...
- **Windows-job hygiene:** ~~`CARGO_TARGET_DIR: C:\t`~~ **- corrected during PR A, do not reinstate.** `rust-cache` never reads `CARGO_TARGET_DIR`; it derives its cache path as `path.join(workspaceRoot, targetInput)` (`src/config.ts`), which for an absolute target yields the nonexistent `.\C:\t`, so the job would silently cache nothing on every run. The MAX_PATH worry is also not binding: lance's own Windows CI builds the same `aws-lc-sys` from a deeper path (`D:\a\lance\lance\rust\target\...`) with the default target dir. If `LNK1104`/`os error 206` ever appears, pair a short target dir with a hand-rolled `actions/cache`, never with rust-cache. **What IS required before checkout: `git config --system core.longpaths true`** - pond's longest tracked path is 456 chars (a claude-desktop fixture nesting an encoded project slug inside a session dir) and git on Windows fails the checkout outright with "Filename too long" without it, regardless of the `LongPathsEnabled` registry key. `Swatinem/rust-cache` pinned >= v2.9.2 (v2.9.2 fixes a Windows-only path-validation regression) with `save-if` covering **both** `refs/heads/main` and `workflow_dispatch` - a dispatch-only save is what makes iterating on a Windows branch viable, since the job never runs on ordinary PRs. sccache and kache were evaluated and rejected: both are rustc wrappers, so neither caches build-script *execution*, and this graph's slowest native work (`aws-lc-sys` under cmake + NASM) happens exactly there - which rust-cache does restore as part of `target/`. Treat the warm cache as load-bearing, not incidental: Lance's full-suite Windows job runs 20-21 min warm on a paid 16-vCPU `windows-latest-4x` runner, which projects to roughly 50-75 min on the free 4-vCPU `windows-2025` - a cold run plus the second `--profile dist` build can plausibly approach the 120-min ceiling. Confirm on the first runs that rust-cache picks up the `CARGO_TARGET_DIR: C:\t` override, and that the repo's 10 GB Actions-cache LRU budget is not thrashing (a lance-class Windows `target/` can evict the Linux jobs' entries and vice versa). If the cache proves unreliable, sccache is the lever.
