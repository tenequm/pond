...
- It is broken before that, too: the whole CLI future runs on the main thread, and Windows defaults the main-thread stack to 1 MiB (unix mains get 8 MiB) - constructing and polling the giant async state machine overflows it before argument parsing, so even `--version` crashes (found by PR #147 on msvc builds; assume the gnu artifact is affected too until CI proves otherwise). Fix in 3.4.
...
- **Main-thread stack (startup blocker; adopt PR #147's fix):** run the tokio runtime and the CLI future on a dedicated named thread with a 16 MiB stack (`std::thread::Builder::stack_size`, `block_on` on that thread, propagate the join result as the exit code). `#[tokio::main]` polls the giant CLI state machine on the 1 MiB Windows main thread and overflows before any code runs - and `Box::pin` does not help, since the future still materializes as a stack temporary first. Behaviorally identical on unix (which already had ample stack).
...
- **Registration:** generate task XML, register with `schtasks /Create /TN "pond-sync" /XML <file> /F`, remove with `/Delete /TN "pond-sync" /F` (delete-then-create for updates; schtasks has no in-place update). Write the XML file as UTF-16LE with a BOM (matching its `encoding="UTF-16"` declaration and Task Scheduler's own export format): schtasks reads a BOM-less file through the ANSI code page, and PR #147 verified the failure mode - a non-ASCII path mojibakes into a task that registers fine and silently does nothing on every tick. XML shape (settled by research; do not use COM - the Rust `planif` crate cannot express minute-level repetition, and comparable tools migrated away from COM):
...
- **Until this ships:** the existing `not(unix)` stub already says "run `pond sync` from Task Scheduler" (`schedule.rs:93`; the `start()` stub at `:107` does not) - phase 1's delta is small: point both stubs at the docs snippet carrying the exact task XML and `schtasks` line, so unattended sync is achievable before it is automated.
...
- VBScript deprecation (why the PR's `.vbs` shim is interim and `pondw.exe` is the end state): [Microsoft's timeline](https://techcommunity.microsoft.com/blog/windows-itpro-blog/vbscript-deprecation-timelines-and-next-steps/4148301) - feature-on-demand now, disabled by default later, then removed.
