...
- **Re-scoped delta (what this plan still owns, in order):**
  ...
  3. **PR C - 3.5 adapters:** claude-code decoder + Windows fixture gate, claude-desktop MSIX glob. **SHIPPED**, fixture gate included:
     ...
     - **Still open:** the Cowork Windows *ingest* path. That machine's `local-agent-mode-sessions` holds plugin and scheduled-task state but no `audit.jsonl`, so agent mode has never run there and only the probe half is verified.
