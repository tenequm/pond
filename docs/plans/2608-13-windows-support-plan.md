...
- **Re-scoped delta (what this plan still owns, in order):**
  ...
  3. **PR C - 3.5 adapters:** claude-code decoder + Windows fixture gate, claude-desktop MSIX glob. **SHIPPED**, except the fixture gate, which needs a real Windows capture:
     ...
     - **The slug rule is confirmed on real native Windows.** A live `%USERPROFILE%\.claude\projects` capture stores a `C:\Users\<user>\<project>` cwd as `C--Users-<user>-<project>` - exactly what the bundle rule and the decoder produce, in both directions. Also checked on that corpus: transcripts are **LF, not CRLF** (no reader change needed), and no `subagents/` directory is present, so that path is still only covered synthetically.
     - **Still open:** committing a scrubbed Windows fixture (the live capture is personal transcript content, so it cannot be committed as-is), and the Cowork Windows *ingest* path - that machine's `local-agent-mode-sessions` holds plugin and scheduled-task state but no `audit.jsonl`, so agent mode has never run there and only the probe half is verified.
...
> Status: shipped in PR C except the Windows fixture gate. The corrections the implementation forced - the slug rule read off Claude Code's bundle, the posix underscore bug it exposed, pi's UNC divergence, and opencode/oh-my-pi needing nothing - are recorded under PR C in section 2.1; read that before trusting the bullets below.
