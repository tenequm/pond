...
- **Re-scoped delta (what this plan still owns, in order):**
  ...
  2. **PR B - remaining 3.4 (SHIPPED).** PATHEXT resolver + resolved-path claude spawn, cliclack floor at 0.5.6, reserved-name validation, Windows commit-retry arm, self-heal quarantine message, rowmap sweep polish, `home_dir` delegation, the path/URL seam audit + UNC gate, and the 7.8 spec correction. Findings and deliberate non-items from the pass:
     - **The dir-layout merge-time ask landed with PR #147**, so PR B owed nothing there: `config.rs` already splits `%LOCALAPPDATA%\pond\{data,cache}` / `%APPDATA%\pond\config.toml` and honors an absolute XDG override on every platform.
     - **The UNC gate is an `anyhow::bail!` at `StorageUrl::parse`, not a `pond::Error` variant.** "Typed" in the 3.4 bullet meant *gated at the seam instead of failing deep inside Lance*; `pond::Error` is the wire-boundary type and every sibling rejection in `parse` is a bail. Mapped network drives cannot be told from a local drive without a Win32 call (pond denies unsafe), so the message names them rather than detecting them.
     - **New finding, fixed: `expand_home_under` did not expand `~\`.** shellexpand only knows `~/`, while lance's own `expand_tilde_path` has a `#[cfg(windows)]` arm for `~\` - so a Windows config saying `~\pond` would have created a literal `~` directory while lance resolved the same string to the home dir. Verified in the same pass that shellexpand does **not** treat `\` as an escape, so drive paths survive byte-identical.
     - **`syncstate::pond_state_dir()` returning `state_root()` unsuffixed on Windows is left as-is.** It looks asymmetric (a user-set `XDG_STATE_HOME` gets pond's files directly in it, with no `\pond` level) but it is what makes the scheduler's pin-and-reread contract round-trip, it is documented at the definition, and changing it would break the registration PR #147 shipped. Revisit with the phase-3 `--state-dir` argument pinning (5.2), not before.
     - **Known sharp edge, deliberately not touched:** widening `validate_path_id` makes an old control-flow shape newly reachable. `opencode.rs:584` and `:613` propagate the error with `?`, so one bad id aborts the whole directory walk rather than failing that session (the sibling call sites at `hermes.rs:429` and `opencode.rs:500` attribute per-session and continue). The old `/`,`\`,`..` predicate could never match a `file_stem`, so nothing reached it before. Real opencode ids are `ses_<base62>`, so the trigger probability is ~0 - changing error propagation in that walk is its own PR, not PR B's.
     - Not automatable, still open: the 3.4 manual wizard verification (Ctrl-C inside a cliclack prompt in Windows Terminal). The cliclack floor is the fix; a human still has to confirm it.

     Plus a one-line spec correction: 7.8's Task Scheduler parenthetical lands with the PR naming `AllowStartIfOnBatteries` (not a real element - the XML writes `DisallowStartIfOnBatteries=false`) and crediting `Hidden` with the windowless posture (it is UI-only; the `.vbs` shim's WindowStyle=0 is what suppresses the console). Correct both, and say "windowless launch shim" rather than naming `.vbs`, so the sentence survives the phase-3 `pondw.exe` swap untouched - only the `schedule.rs` module header, which does name the mechanism, changes then. Doing it here rather than with 5.2's 7.8 rewrite: that rewrite is phase 3, too late to leave a nonexistent element name in the spec.
  3. **PR C - 3.5 adapters:** claude-code decoder + Windows fixture gate, claude-desktop MSIX glob. **SHIPPED**, fixture gate included:
     - **The claude-code slug rule is no longer a model - it is read off Claude Code's own bundle.** `vJ(cwd) = join(projectsDir, _b(cwd))` with `_b = A.replace(/[^a-zA-Z0-9]/g, "-")`: every non-alphanumeric character becomes `-`. Cross-checked against 181 real project directories (178 exact; the 3 misses are a worktree `cwd` drift and two `ssh-<uuid>` remote sessions, which are not path-derived at all) and against the 14k-session corpus, which contains 31 underscore-bearing project paths and zero with a space.
     - **That made 3.5's "binding hazard" bullet only half right, and turned up a live posix bug.** `encode_project` was `['/', '.']`, so `_` (and a space) survived into the slug: every restore of an underscore-bearing project has been writing to the wrong directory on macOS and Linux too. Fixing to the real rule closes the Windows `\`/`:` hazard and that bug in one change.
     - **`\` counts in UTF-16, not chars.** The JS regex is un-`u`-flagged, so an astral character collapses to *two* dashes. `len_utf16()` per character, not one dash per `char`.
     - **NFC normalization does not apply.** Claude Code normalizes the *memory* path (`I_1`), not the session path (`vJ`) - so pond must not normalize either. 3.5's bullet said NFC; the bundle says otherwise, and the bundle wins.
     - **pi's real divergence was not `\`/`:` - those were already mapped - it was UNC.** `sessionDirectoryName` strips exactly ONE leading separator (`/^[/\\]/`, no `g` flag); pond used `trim_start_matches`, which eats both, so every `\\host\share` project encoded to a directory pi would never look in. One-character class fix.
     - **opencode needed nothing, contra the 3.5 bullet.** Its `encode_project` already collapses every non-alphanumeric, and the result lands in a JSON `projectID` field - never a path segment. Verified rather than assumed; the module now says so, so it does not get "fixed" later.
     - **oh-my-pi needed no probe change either:** `~/.omp/agent/sessions` is home-relative, so PR B's home fix already revived it. Only the PR #147 field verification (331 sessions on Windows 11) was missing from the module.
     - **3.5's MSIX claim is wrong for the mainline installer, and that inverted the probe order.** Checked on a real Windows 11 machine running Claude 1.30096.1: `Get-AppxPackage *Claude*` returns nothing and `%LOCALAPPDATA%\Packages` has no Claude entry at all - the app ships as a per-user Squirrel install under `%LOCALAPPDATA%\AnthropicClaude`, so Electron's `userData` is never virtualized. `%APPDATA%\Claude\local-agent-mode-sessions` exists and is the real path. It now leads; the MSIX glob stays as a secondary probe for the Store channel, still unverified against a packaged install, and still written to find nothing rather than find wrong.
     - **The fixture gate is CLOSED.** Rather than scrub a personal corpus, the fixture was *generated* on the Windows box: two sessions written by Claude Code 2.1.232 itself from a `cwd` of `C:\dev\pond fixture_demo.v2` - one path deliberately carrying a drive colon, backslashes, a space, an underscore and a dot, so a single capture pins every character class the rule collapses. Claude Code stored it as `C--dev-pond-fixture-demo-v2`, matching `encode_project` exactly. Committed at `tests/fixtures/adapter/claude_code/windows-projects/`, covering both a plain session and a two-subagent session (`<parent>/subagents/agent-<hash>.jsonl` + `.meta.json`, identical to the posix layout). Only edit: the `%LOCALAPPDATA%\Temp` Task output paths had their user component replaced with `user`; nothing the tests assert. The `cwd`-less fallback half is derived in-test by stripping that one field, since no natural corpus contains a `cwd`-less transcript.
     - Also settled by that capture: transcripts are **LF, not CRLF** (no reader change needed), and the slug is stable across Claude Code 2.0.76 and 2.1.232.
     - **Cowork Windows *ingest* is unverified and deliberately not chased.** The store name is the reason: `local-agent-mode-sessions` fills only for **local** agent mode, and Cowork runs in the cloud by default - a Cowork session launched on the box wrote no `audit.jsonl` at all. So the adapter's Windows probe is verified while its ingest is not, and closing that would mean driving the desktop app into local mode purely to manufacture a fixture. The ingest path itself is platform-independent (the same `audit.jsonl` reader the macOS fixture already covers); only the store location was ever Windows-specific, and that is the half now tested.
     - **Two things shipped in PR C that are not Windows items**, both found by driving the real tools on the Windows box rather than by reading code. (a) `oh-my-pi` subagents were ingesting as top-level sessions: omp keeps a session's children in an artifacts directory named after its own file and the child header carries no parent field, so pond saw four independent sessions where there were two plus two children, and they ranked in default search against the rule that excludes subagents. The parent is now read from the neighbouring file's own header - not parsed out of the directory name, which invents a parent for nested children since agent ids keep `_`. (b) The kache build-cache version is pinned in `windows-bootstrap`: the action's default resolves latest at run time, and 0.14.0's cache-key bump cost a 138 min run against 9.5 min warm with no commit in this repo to explain it.
  4. **Phases 2 and 3 - SHIPPED** except SignPath (5.1) and the first winget submission (4.5.4); see the status notes on each section.
...
- **Budgeted work item - unix path literals in the test suites.** The red-team counted ~105 unix-absolute-path string literals in `packages/pond/src` unit tests and 19 in `packages/pond/tests` that fail on Windows independent of any Windows code change (e.g. `config.rs:1003-1021` - `PathBuf::from("/xdg").is_absolute()` is false on Windows so the XDG leg silently skips; `config.rs:911-916` - `file:///tmp/...` has no drive letter so `to_file_path()` errs). Concentrations: `adapter/claude_code.rs` (~33), `config.rs` (~23), `adapter/discovery.rs` (8), `main.rs` (7). This is the single largest phase-1 line item: audit and parameterize (platform-conditional expected values, or drive-lettered fixtures under `cfg(windows)`), file-by-file, before the first green Windows run is achievable. Budget it explicitly; do not fold it into "add a CI job". **RESOLVED in PR A - this line item cost nothing.** The first `windows-verify` run passed the full suite on `windows-2025` with **zero** test changes, so the ~124-literal count was purely an upper bound and PR #147's calibration was right: those literals sit in cfg-gated or non-asserting positions. Do not budget for this again.
...
- **Windows-job hygiene:** ~~`CARGO_TARGET_DIR: C:\t`~~ **- dropped during PR A.** The MAX_PATH worry it guarded is not binding: lance's own Windows CI builds the same `aws-lc-sys` from a deeper path (`D:\a\lance\lance\rust\target\...`) with the default target dir. (It was also outright incompatible with the `Swatinem/rust-cache` the plan originally specified, which never reads `CARGO_TARGET_DIR` and derives its path as `path.join(workspaceRoot, targetInput)` - for an absolute target that yields the nonexistent `.\C:\t`, silently caching nothing. Moot now that the cache is kache, which restores into `target/` wherever it is, but recorded because it is a trap worth not re-treading.) **What IS required before checkout: `git config --system core.longpaths true`** - pond's longest tracked path is 456 chars (a claude-desktop fixture nesting an encoded project slug inside a session dir) and git on Windows fails the checkout outright with "Filename too long" without it, regardless of the `LongPathsEnabled` registry key. **The cache is `kunobi-ninja/kache-action` against S3, NOT GitHub's cache - operator decision, do not revisit.** GitHub caps a repository at 10 GB (raisable only as a paid metered SKU, ~$0.056/GB/month) and scopes entries per branch so siblings cannot share; pond's cache exceeds that budget and Windows work happens on feature branches. kache writes content-addressed artifacts to the same Hetzner bucket the dist build already uses (`ttq`, prefix `kache/pond`, `KACHE_S3_*` secrets, `s3-endpoint: https://nbg1.your-objectstorage.com`), with no ceiling, no GitHub storage billing, and keys portable across branches/runners/checkouts. Set `github-cache: false` so the fallback is explicit, `manifest-key` to a per-build-variant value (the triple-keyed default makes every build sharing the bucket clobber each other's prefetch manifest), and `pr-comment: false` (it would need `pull-requests: write`). Its post step is `success() || failure()`, so a red run still saves - the case that matters, since Windows iteration means a platform test failing after an hour of compiling. **Accepted limitation:** kache is a `RUSTC_WRAPPER`, so it caches rustc units but not build-script *execution* - `aws-lc-sys` under cmake + NASM, plus `ring`, `blake3`, `zstd-sys`, recompile their native halves every run. Treat the warm cache as load-bearing: measured, the cold test leg alone is 76 min on the free 4-vCPU `windows-2025`, so `timeout-minutes` is 180, not 120.

### 3.3 CI job

**Two** jobs in `ci.yml` - `windows-verify` (the full test suite) and `windows-dist` (the shipped msvc zip) - sharing a `.github/actions/windows-bootstrap` composite action, both on runner `windows-2025`. Split because one serial job measured 46 min warm (20.7 test + 23.8 dist); parallel, they start together on separate hosted runners so Windows contributes the longer of the two to a release rather than the sum, and the composite action stops a protoc skew between them producing two differently built binaries from one commit. They MUST carry different kache `manifest-key`s or each overwrites the other's warm-prefetch manifest. Runner note (note: `windows-latest` = `windows-2025` = `windows-2025-vs2026` - Windows Server 2025 with **VS 2026** as of the June 2026 image migration; pin `windows-2022` only if VS 2022 turns out to be needed). The image ships Rust 1.97.1/rustup, CMake, 7-Zip - and neither protoc nor NASM (verified against the image software list).
...
- **Triggers:** add `workflow_dispatch` to the workflow `on:` block (side effect to accept: `build-and-test`/`flake-check` carry no event condition and will also run on dispatch; `release-prep` and downstream stay push-gated). Job condition (release-plz's release-PR branch prefix is the default `release-plz-`; pond's `.github/release-plz.toml` does not override it - verified):
  ...
  **Triggers differ between the two.** `windows-verify` runs on ordinary same-repo PRs as well (measured 12.3 min warm - there is no reason to defer a Windows regression to release time; the original plan deferred it only on a 50-75 min estimate made before kache). Fork PRs are excluded: they get no secrets, so kache has no S3 credentials. `windows-dist` stays release-gated - dispatch, main push, or a `release-plz-` PR - because it produces an artifact nothing consumes outside a release. To make the release gate binding, mark **both** as required status checks in branch protection; GitHub treats a skipped required check as satisfied, so `windows-dist` only actually blocks release PRs. Neither takes a `needs:` - both start at t=0 in parallel with the Linux jobs. `windows-verify` is `cancel-in-progress: true` (a test job, superseded by a newer push, like `build-and-test`); `windows-dist` is **`cancel-in-progress: false`**, the `dist-build` rule, because cancelling an artifact producer mid-release cancels the publish with it. `timeout-minutes: 180` (measured: the cold run took 138.6 min - 76.4 test + 61.0 dist - on the free 4-vCPU runner), a `concurrency` group with **`cancel-in-progress: false`** (this job produces a release artifact, same rule as `dist-build`; cancelling it mid-release cancels the publish with it), and `env: BAZEL_REMOTE_AUTH: ""` (same rule as `flake-check`/`macos-verify`: hosted runners compiling third-party build scripts get no cache credentials).
...
- **Watch item riding the probes: `url` > 2.5.4 Windows path regression - CLOSED during PR A, no action needed.** Verified against the pinned source: lance v8.0.0 contains `3de00ee05 feat: upgrade url crate, rework temporary directory utils (#4860)`, and lance's own `Cargo.lock` at that tag pins `url 2.5.8` - byte-identical to pond's. Upstream reworked for the newer url and ships it, so the `url = "=2.5.4"` pin is not a lever pond needs. Original analysis retained below for context. pond's lock has `url 2.5.8`, and [object_store#499](https://github.com/apache/arrow-rs-object-store/issues/499) (filed by westonpace of LanceDB) shows `LocalFileSystem` failing on Windows paths with url >= 2.5.5 - a `path_segments_mut().extend` behavior change, [rust-url#1077](https://github.com/servo/rust-url/issues/1077) (open; fix PR rust-url#1108 open). pond's own constructions use `Url::from_file_path` (the safe direction), but the Lance stack rebuilds paths from `file://` URLs internally. Lance merged a url-upgrade rework ([lance#4860](https://github.com/lance-format/lance/pull/4860)) - verify at implementation time whether lance 8.0.0 contains it. If the Windows suite hits confusing local-path failures, pinning `url = "=2.5.4"` in the lock is the first diagnostic lever.

### 3.4 Platform correctness fixes

> **Status: shipped.** Every bullet below has landed - the main-thread stack, `_command` shell, and lock-holder items with PR #147; the rest with PR B (see 2.1 for the findings and the deliberate non-items). The only open item is the last one, which is manual by nature. Bullets are kept as written because they carry the verified upstream reasoning, not because work remains.
...
- **Manual verification item - DONE, and it turned out to be automatable.** `pond init` end-to-end plus Ctrl-C during a cliclack prompt. Driven over SSH under a Windows ConPTY: ship a `.ps1`, run it with `pwsh -File`, send `\x03` once the prompt renders. The prompt cancels, the wizard prints `Cancelled - nothing written` and exits 1, and the sandbox root does not exist afterwards - so cancel is side-effect free, not merely clean-looking. (Do NOT inline the env assignments into the `ssh` command: pwsh splits `$env:X="..."` into separate tokens and the run dies confusingly. A script file is what makes this work.) ConPTY is not literally Windows Terminal, so a belt-and-braces human check remains available, but the cliclack 0.5.6 native Ctrl-C path is demonstrated working on real Windows.

### 3.5 Adapters

> Status: shipped in PR C, fixture gate included. The corrections the implementation forced - the slug rule read off Claude Code's bundle, the posix underscore bug it exposed, pi's UNC divergence, and opencode/oh-my-pi needing nothing - are recorded under PR C in section 2.1; read that before trusting the bullets below.
...
> **Status: ALL MET.** `windows-verify` green in CI on real hardware; the manual gate below closed on the Windows box (`pond init`, first sync into a local store with embeddings on CPU, `pond search`, and `pond mcp` registration that `claude mcp list` reports as Connected); spec 2.4 amended; README carries the Windows + WSL note; the binstall override is renamed and the gnu zip is gone. Phase 1 is complete - the remaining work is phases 2 and 3, most of which is gated on operator accounts rather than code (section 4.5).

- `windows-verify` green (full test suite + both probes) and `windows-dist` green (dist build + launch smoke + zip).
...
> **Status: shipped except the first winget submission.** 4.2, 4.3, and 4.4 landed with PR #160; Scoop was verified end to end on the Windows box (`scoop bucket add` -> `scoop install pond` -> `pond --version` -> `scoop uninstall`, hash and all four completion scripts confirmed). 4.1's automation shipped in the same PR but stays inert until a manifest exists upstream: winget-releaser hard-fails while the package is absent from winget-pkgs, and the install docs say so rather than advertising a command that returns "No package found". Deviations from the section as written: the workflow is a job in `ci.yml`, not a reusable workflow, and it is called from `publish-release` rather than triggered `on: release` - release-plz publishes the release before the binaries are uploaded to it, so a release-triggered run would see an empty asset list.

### 4.1 winget

- **First submission is manual and interactive** (hard requirement - winget-releaser requires at least one version to already exist in winget-pkgs): `komac token add` (the classic PAT), then `komac new` - Komac v2.16 documents `new` with **no arguments**, it is an interactive wizard; there is no documented non-interactive form, so budget a human at a terminal for this one-time step. Submit **0.14.11 or later, never 0.14.10** - that zip predates `pondw.exe` (5.2) and `pond schedule start` refuses to register without it. Manifest shape: `InstallerType: zip`, `NestedInstallerType: portable`, and **both** binaries declared - `NestedInstallerFiles: [{RelativeFilePath: pond.exe, PortableCommandAlias: pond}, {RelativeFilePath: pondw.exe}]`. Declaring `pondw.exe` is load-bearing: winget's ZIP spec places down the *declared* nested files (its uninstall record is "the list of files that were created and placed down", and multiple entries exist precisely to support a suite of portable exes), so an undeclared `pondw.exe` would leave every winget install with a broken scheduler. It costs only a cosmetic `pondw` PATH alias, and is harmless even if winget turns out to copy the whole tree. **Get this right first time:** winget-releaser builds each later version from the existing manifest as a base, so the shape submitted here is inherited by every automated update. Verify with `winget install --manifest` on the Windows box before submitting upstream. A portable winget install provides PATH, Add/Remove Programs, and `winget uninstall` - no MSI, no signing required (verified: winget validation does not require signed binaries, and winget downgrades MOTW to trusted-zone after hash verification, so no SmartScreen on install).
...
- **The first release after the automation lands shows a RED `winget-publish` job, on purpose - do not "fix" it.** winget-releaser needs a base version already present in winget-pkgs, so it fails until the manual submission above has happened; nothing depends on the job, so the release itself still completes. A guard was considered and rejected: querying winget-pkgs for the manifest path and skipping when absent trades a *loud one-time* failure for a *silent permanent* one - a typo in the identifier, a rename, or an upstream path change would make the step skip, the job go green, and winget quietly stop receiving releases forever. The condition it would guard is true exactly once in pond's lifetime. Let it fail loudly, then do the manual submission.
...
1. **DONE.** Classic PAT with `public_repo` scope, saved as `WINGET_PAT` repo secret (fine-grained PATs unsupported by winget-releaser).
2. **DONE.** Fork `microsoft/winget-pkgs` under `tenequm`.
3. **DONE.** Create `tenequm/scoop-bucket` from the template, populated and topic-tagged.
4. **OPEN - the only thing between here and a working `winget install`.** First submission. Not the interactive `komac new` this section assumed: komac 2.16 blocks on a token prompt even under `--dry-run` and has no flag for the nested-installer type, so the three manifests were hand-written against schema 1.12.0 (modelled on `junegunn.fzf`, the closest analog - a portable zip with the exe at the archive root) and pushed to the fork on branch `pond-0.14.10`. Deliberately re-pointed at the next release rather than submitted as 0.14.10: one upstream review cycle instead of two, and it avoids making the pre-pondw build pond's winget entry point.

## 5. Phase 3 - parity

> **Status: 5.2 and 5.3 shipped; 5.1 is the only open item in the plan.** The stated sequencing did not survive contact: signing-first existed so a task-registering binary would not be unsigned, but PR #147 already shipped the Task Scheduler backend unsigned and section 2.1 accepted that, so the scheduler and durability work no longer wait on SignPath. 5.1 is an application to a third party with weeks of review and gates nothing - winget requires no signing (it downgrades MOTW to the trusted zone after hash verification), and Scoop and binstall do not care.

Sequencing inside the phase as originally planned: **signing -> scheduler -> durability**. Signing first because registering a scheduled task from an unsigned binary is a documented Defender-deletes-the-binary trigger (resticprofile ships that exact warning).

### 5.1 Code signing

> **Status: DEFERRED, and this section is a decision record, not a queued task.** Re-researched 2026-08-17 against SignPath's live terms and Microsoft's current guidance; the original recommendation below did not survive it. Do not start an application without revisiting this.

**What signing would actually buy pond: very little.** winget downgrades MOTW to the trusted zone after hash verification, and Scoop and binstall never set it - so only a browser-downloaded release zip triggers SmartScreen, the minority path for a CLI. Microsoft's own comparison table says signing does not remove even that immediately ("Reputation builds over time; initial warnings expected"), for Azure Artifact Signing and OV certificates alike. The "no EV cert" call below is confirmed.

**What it would cost: a permanent gate on every release.** SignPath's terms require that "every release needs manual approval for signing". pond's release path is fully automated end to end, so this inserts a human step into every future release, permanently, to fix a minority download path once. Standing obligations on top: a "Code signing policy" section on the project home page naming team roles and a privacy policy, MFA on every account, and enforced PE metadata restrictions (consistent product name and version attributes - verify pond emits them). SignPath also reserves the right to reject: downloadable executables need "a certain verifiable reputation".

**The paid alternative is geographically closed.** Trusted Signing was rebranded **Azure Artifact Signing** and reached GA in early 2026 at ~$9.99/month. Public Trust certificates cover *organizations* in the US, Canada, EU and UK, but *individual developers in the US and Canada only* - so an EU individual cannot use it. Signing through an EU legal entity is the one option that is operationally better than SignPath: it keeps the release fully automated and puts pond's own identity on the certificate rather than making SignPath Foundation pond's publisher of record.

**Revisit when** a Defender false positive shows up in the field, or direct-zip downloads become a real share of installs. Until then signing gates nothing: winget requires no signing, and Scoop and binstall do not care.

Original recommendation, kept for the reasoning it carries: apply to [SignPath Foundation](https://signpath.org) (free OSS tier; requires OSI license - Apache-2.0 qualifies - MFA, a published signing-policy page, defined roles, and demonstrated usage, which phases 1-2 accumulate). Integrate `signpath/github-action-submit-signing-request` into the Windows release leg. Do not buy an EV cert (EV no longer buys SmartScreen reputation - verified against Microsoft's current guidance).

### 5.2 `pond schedule` Windows backend

> **Status: SHIPPED (PR #161).** Verified on the Windows 11 box through `schtasks` rather than pond's own reporting: registration writes `pondw.exe` as the action with the full argument line, no `wscript` anywhere, and a forced run returns `Last Result 0` with the child's stdout in the log and `Last Result 2` with its stderr when the child fails - the exit-code signal the `.vbs` chain structurally could not carry. Deviations and findings:
> - **`pondw.exe` is a launcher, not a second copy of the CLI.** The plan's "~20 lines, calls the same entrypoint" is not reachable: the entrypoint lives in the binary crate, so sharing it means either moving all of it into the library or `#[path]`-including `main.rs` twice, and the second compiles the whole CLI again against a dist leg already at 61 min. The launcher form delivers every property 5.2 asks for at ~50 lines.
> - **The bin is behind a `windows-launcher` feature.** An unconditional second `[[bin]]` makes `cargo binstall` demand `pondw` in *every* archive, and the unix tarballs carry only `pond` - reproduced against the real v0.14.10 tarball. Binstall ignores a bin whose required-features are unmet, so unix resolves `pond` alone and no dead binary is built there. `ci.yml`'s dist build and `pond:test-msvc` both enable it.
> - **Two silent-failure bugs found in review, both fixed:** a state dir ending in a separator emitted `--state-dir "C:\dir\"`, where `CommandLineToArgvW` reads `\"` as an escaped quote and swallows the rest of the line; and `pondw` refused to spawn at all when it could not open the log, turning a missing state dir into a permanent no-op.
> - **Shim-resolved installs need the fallback:** winget puts a symlink on PATH and Scoop a shim exe, neither with `pondw.exe` beside it, so `pondw_bin` falls back to the running binary's own directory. Confirmed for Scoop's shim shape; the end-to-end (install the zip, then `pond schedule start`) is only runnable once a release carries `pondw.exe`.
> - **Upgrade action, must reach the changelog:** an *already registered* task keeps its old `.cmd`+`.vbs` action chain - the swap only rewrites the task when `pond schedule start` runs again. Upgrading pond does not re-register it. The release note carrying this binary is the only place an existing Windows user will learn to re-run it, so it belongs in the changelog entry, not just here.
...
- **`pondw.exe`:** `src/bin/pondw.rs` (~20 lines), `#![cfg_attr(windows, windows_subsystem = "windows")]`, calls the same entrypoint as `pond`; built and shipped in the zip only on Windows. It has no console, so scheduled-run output goes to the sync log file in the state dir (same shape as the launchd/systemd backends); `pond schedule logs` reads it. The swap also recovers a signal the shipped chain cannot carry, measured on hardware: the `.vbs` shim launches the wrapper fire-and-forget (`Run ..., 0, False`), so Task Scheduler's `Last Result` records wscript's exit rather than pond's - a failing sync still reports `0` and the log is its only trace. A direct `Exec` on `pondw.exe` makes `Last Result` the real exit code again.
...
- **Until this ships:** nothing to bridge - PR #147 lands a working Task Scheduler backend at merge, so unattended sync exists from phase 1 onward; the swap above is invisible to users. (The pre-PR `not(unix)` stubs this bullet used to patch are deleted by the PR.) Verified end to end on the Windows 11 box against a debug build of 26d27e5, asserting through `schtasks` rather than pond's own reporting: registration writes the expected XML (`PT5M` repetition, no `Duration`, both battery settings, `StartWhenAvailable`), `status` recovers the cadence, a second `start` is a no-op, a forced `/Run` puts pond's output in `sync.log` (the check that rules out the silent-no-op encoding mode), and `stop` leaves no task behind.

### 5.3 Windows durability wrapper

> **Status: SHIPPED (PR #162).** Landed smaller than written: rather than a second `cfg(windows)` wrapper, the existing one is attached per *backend* instead of per platform, and only the parent-directory fsync is gated to unix. Two bugs surfaced on real hardware that no amount of reading would have caught - the module carried its own `#[cfg(unix)]`, so removing the gate at the call site alone did not compile on Windows; and `FlushFileBuffers` returns `ERROR_ACCESS_DENIED` on a read-only handle, so every wrapped write failed with os error 5 until the file was opened for write first. The spec change also landed smaller: one `local-store-durability` rule stating the platform delta inline, not the separate `windows-store-durability` rule this section proposed, which would have re-derived rationale the existing rule already carries.
>
> **Two review findings worth keeping, because both are invisible from the diff:**
> - **The wrapper's errors must keep the `io::Error` reachable as a source.** `durability_error` formatted it into a string, and `is_transient_sharing_violation` classifies by downcasting to `io::Error` - so a contended flush was unretryable. Latent while the flush opened read-only on unix (an `O_RDONLY` open never contends); live the moment Windows opens for write, which is exactly what raises `ERROR_SHARING_VIOLATION` when a scanner or sibling reader holds the object just published.
> - **`rename_opts` is the deliberate exception and must stay one.** Its flush runs *after* the rename published the name, so retrying it re-attempts a version that landed, and OCC rebases that into re-appending rows already committed. That call site flattens its error on purpose so the retry classifier cannot promote it. Do not "fix" the inconsistency by making all four call sites alike - a test pins both halves.
>
> **CI gap closed alongside:** `pond:lint-msvc`. Windows-only code had never been clippy-gated - `lint` runs on Linux and never compiles a `cfg(windows)` block, and `test-msvc` runs `cargo test`. That gap is how an unused import reached the branch.
>
> **Cost on Windows, measured:** free. A per-commit append sweep A/B'd against `origin/main` on the same machine costs **under 1 ms per commit at every batch size** (12.4 -> 12.5 ms at batch 1; 23.4 -> 24.2, +3.4%, at batch 512). Windows trails an M1 Max by 21-55% on the same sweep, but the no-wrapper arm trails by the same margin, so that gap is the machine, not the wrapper. Full tables and environments in [docs/benchmarks/results.md](../benchmarks/results.md). Note the sweep must be sourced from a *small* store: `append_absent_rows` rescans the source per call, so against the real 11 GB corpus every commit pays a ~6 s full-column scan that buries the write side entirely.
...
## Appendix A - Windows job and publish-release fragments (verified wiring)

> **Stale shape:** the fragment below is the ORIGINAL single-job design. It shipped as two jobs (`windows-verify` + `windows-dist`) over a shared `windows-bootstrap` composite action - see section 3.3. Read `ci.yml` for the real thing; this is kept only for the step-level rationale.
...
### A.1 The Windows job (as originally drafted - single job)
...
```yaml
  publish-release:
    needs: [dist-build, macos-verify, windows-verify, windows-dist]
    # ... existing config unchanged ...
    steps:
      # ... existing checkout etc. ...
      - uses: actions/download-artifact@<pin-sha> # v8
        with: { name: dist-all, path: dist }
      # The msvc zip is built natively by windows-dist, not by the moon dist
...
flake-check ------+-> release-prep -> dist-build -+-> macos-verify -+
                                                  +-----------------+-> publish-release
windows-verify -----------------------------------------------------+   (tests; no needs:, t=0)
windows-dist -------------------------------------------------------+   (zip;   no needs:, t=0)
```
