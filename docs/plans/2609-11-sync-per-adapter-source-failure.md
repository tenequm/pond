...
- `pond status` was already tolerant of adapter read failures through `opened.plan(&oracle).await.ok().flatten()` plus a per-adapter `error` column from `discover()` (main.rs:6312-6326), but its absent-root classification still needed the shared `reason` token.
- OpenClaw deletion reconciliation tolerates a missing root: `list_agents` returns `Ok(vec![])` on `NotFound` (`packages/pond/src/adapter/openclaw.rs:425-428`). *Addendum: the root was never the whole story - the per-agent `sessions/` scan below it swallowed every error, which a polish pass turned into a propagating one, which turned an agent dir with no readable `sessions/` into a whole-run abort that discarded other adapters' committed work. It now probes the same way its sibling reader does and reports a real read failure without failing the run, per this pass's own rule that ambiguity resolves to preserve.*
...
6. **Wording pinned** (so impl and tests agree):
   ...
   - Real-sync JSON summary gains an additive field, present only when non-empty:
     ...
     on BOTH the ok and error summary documents. Additive per spec 7.8, so no version bump.
...
8. **Out of scope:** making `MultiProgress::println` skip-lines visible off-TTY for ordinary per-file skips; surfacing non-NotFound whole-source errors (permission denied) as adapter-level verdicts in real sync (they keep today's behavior); `pond status` changes; the missing-`path` config error (stays fatal, gets a pinning test). *Addendum: the same PR later shipped the per-file-skip surface after all - `degraded_adapters` (skip count + first reason, JSON + off-TTY stderr) - superseding the first item, and the second only in part: a permission-denied root is now surfaced, but still as a degraded run rather than an adapter-level verdict (`missing_source_root` keeps returning `None` on a `try_exists` error). It also added the shared `reason` token to `pond status`, superseding that out-of-scope item. A later polish pass went further still: `pond status` now short-circuits an absent root on every adapter (five tolerant adapters had reported it as `0 sessions - up to date`), a whole-config run whose every source is absent returns before the store open, and the 7.8 dry-run sentence was corrected to admit that a new destination gets an empty store. Reviewing that pass's own output then reversed two of its decisions and forced a third change. `storage_errors` had joined the `degraded_adapters` populations, but the counter has no write site anywhere in the workspace and cannot get one in the current shape (the store commits a batch wholesale, and real per-row failures already land in `dropped_sessions`), so the population, the spec clause and the dead field were all removed rather than shipped as a health key no run can produce. The all-absent short-circuit was moved back under the sync flock - it skips the store open and the embedder, which are the expensive parts, but taking the lock keeps single-flight true, stops its OK breadcrumb overwriting a concurrent real sync's record, and restores `--no-wait`'s `outcome: "skipped"`. And because that run now leaves no store, `pond status` had to learn to render the configured adapter rows and the last-sync record on an uninitialized store: without it the host this PR is about reported `no data yet - run pond sync`, advising the operator to re-run the command that had just correctly done nothing. Extracting the sync verb out of `main.rs` was considered and declined: the concern is not one contiguous range, so it needs its own PR. The pond-history dig found the old silence was never a recorded decision.*
