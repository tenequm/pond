---
description: pond's MCP tools - pond_search (vector or full-text search over agent sessions), pond_get_session (a whole session by id), pond_get_message (one message with full tool bodies), and pond_sql (read-only SQL over sessions, messages, and parts).
---
