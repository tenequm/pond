...
- `pond optimize` - fold the indexes and embed the backlog (when embeddings are enabled), reporting the expensive conditions it leaves alone with their heal cost; `--full` heals those too; `--only` / `--skip <stage>`
