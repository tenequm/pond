...
- `pond share <session-id>` - publish one session's transcript to a public link (no redaction, `--yes` or interactive confirm required); `--to <url>` for an ad-hoc destination, `--open` to launch it
