...
## Query
- `pond search <query>` - search (`--mode fts|vector`, `--sort-by relevance|recency`); `fts` is the default and `vector` needs [semantic search enabled](/reference/configuration#embeddings); `--explain` for the query plan
