...
`resume` exit `3` is not an error to recover from - it means that session is already resumed. The refusal names the existing file, so the useful response is to open it. With `--format json` every one of these outcomes arrives as `{"error": ...}` on stdout.
