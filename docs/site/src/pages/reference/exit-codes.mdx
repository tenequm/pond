...
Exit `75` asks for a retry rather than a fix. `serve` and `mcp` bound their own resident set ([`[runtime].memory_ceiling`](/reference/configuration#memory-ceiling-and-oom-priority), 4 GiB by default); on a sustained breach the process finishes what it is serving, names the reading on stderr, and exits `75` so a supervisor or an MCP client starts a clean one. Nothing is lost - in-flight requests and any running in-serve sync cycle are drained first. `Restart=always` already covers it; `RestartForceExitStatus=75` makes the intent explicit in a unit that restarts selectively.
