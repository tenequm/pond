...
`POND_EMBEDDINGS_ENABLED=true|false` is the env mirror, which is how containers and CI turn it on without a config file. The value must be the literal `true` or `false`; `1` and `0` fail to load.
