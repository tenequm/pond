...
`pond config path` prints the resolved config location; `pond config show` every setting with its source; `pond schedule logs` the scheduled-sync log. `$XDG_CONFIG_HOME` / `$XDG_DATA_HOME` / `$XDG_CACHE_HOME` / `$XDG_STATE_HOME` override the defaults on every platform.
