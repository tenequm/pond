...
![A live pond corpus, then Claude Code answering a three-month-old debugging question from it](/demo-search.gif)
...
```sh
brew install tenequm/tap/pond   # macOS / Linux; Windows: Scoop, see Install
pond init   # guided setup: storage, adapters, MCP + agent skill, optional schedule
pond sync   # ingest and update indexes
```
...
Everything runs on your machine: ingest, search, SQL, and, if you turn on semantic search, the embedding model (Metal on macOS, CPU elsewhere - no embedding API, no keys). pond sends no telemetry. Its only network traffic is the storage you explicitly configure, plus the one-time embedding-model download if you enabled semantic search, cached locally. Your sessions live in a local directory unless you point pond at your own bucket - and then they go there and nowhere else.
...
- **Agents:** pond is an MCP server - `pond_search` / `pond_get_session` / `pond_get_message` / `pond_sql` over the user's full cross-client history, no per-client integration. Every command is exit-code-gated for CI.

## Next

- [Install](/get-started/install) - Homebrew, Nix, Scoop (Windows), Cargo
