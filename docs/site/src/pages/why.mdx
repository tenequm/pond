...
- **Agents:** pond is an MCP server - `pond_search` / `pond_get` / `pond_sql` over the user's full cross-client history, no per-client integration. Every command is exit-code-gated for CI.
