...
Scoop installs pond as a portable zip: the shim puts `pond` on `PATH`, the bucket updates itself from each GitHub release, and `scoop update pond` upgrades in place. It fetches `pond-x86_64-pc-windows-msvc.zip` (~77 MB download, ~233 MB unpacked), which is attached to every [release](https://github.com/tenequm/pond/releases) - no Scoop, no problem: unpack it anywhere and put that directory on `PATH`; nothing else to install.
...
The zip carries completion scripts for PowerShell, bash, zsh, and fish. Register the PowerShell one by dot-sourcing it from your `$PROFILE`; with Scoop the file is at `~\scoop\apps\pond\current\completions\_pond.ps1`. On a fresh machine `$PROFILE` may not exist yet - create it first (`New-Item -Force $PROFILE`).
