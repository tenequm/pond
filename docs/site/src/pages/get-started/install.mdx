...
macOS, Linux, and Windows. Homebrew on macOS and Linux; Scoop on Windows; a plain [release zip](https://github.com/tenequm/pond/releases) anywhere, or cargo-binstall where a Rust toolchain already exists.
...
# Buckets are git clones: run scoop install git first if git is not on PATH.
...
# Files\NASM yourself), and like binstall it builds pond.exe only - add
# --features windows-launcher for pondw.exe. macOS and Linux vendor protoc.
