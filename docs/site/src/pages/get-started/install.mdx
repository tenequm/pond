...
Scoop installs pond as a portable zip: the shim puts `pond` on `PATH`, the bucket updates itself from each GitHub release, and `scoop update pond` upgrades in place. It fetches `pond-x86_64-pc-windows-msvc.zip`, which is attached to every [release](https://github.com/tenequm/pond/releases) and can also be unpacked by hand.
