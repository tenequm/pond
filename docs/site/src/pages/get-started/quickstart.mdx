---
description: Get started with pond - pond init sets up storage, adapters, and MCP registration; pond sync ingests and indexes your Claude Code, Codex, and OpenCode sessions; then ask any agent about your past work.
---

# Quickstart

Set up, ingest, ask. (`pond` not installed yet? [Install](/get-started/install) first.)
...
```sh
pond sync  # import and index - every enabled adapter
```
...
```sh
pond status  # stored sessions/messages, per-adapter pending counts, index readiness
```
