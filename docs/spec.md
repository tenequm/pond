...
1. **`pond_search`** (`POST /v1/search`) - search; Section 8 specifies retrieval. Returns ranked message hits grouped by session, with the top-scoring matches per session. The output form follows the transport: MCP returns a rendered transcript of the ranked hits, HTTP returns the same hits as structured JSON.
