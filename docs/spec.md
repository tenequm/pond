...
- `pond init` - idempotent setup-and-repair wizard: storage destination (with an end-to-end probe for remote URLs), source adapter selection, MCP registration, and an opt-in sync schedule, written to `config.toml` in a single pass at the end. Every section is flag-answerable for non-interactive use: the destination via the global `--storage-path`, the rest via `--adapters`, `--schedule`, `--skip-mcp`, `--yes`, and `--force` (start from built-in defaults, ignoring existing config). `--yes` alone never schedules.
- `pond sync` - run the import, embed, and update-indexes stages in order. `--only <stage>` runs exactly one stage; `--skip <stage>` omits one. Stages are `import`, `embed`, and `update-indexes`. `--force-embed` re-embeds rows whose `embedding_model` differs from the current model via conditional merge. A positional `<adapter>` syncs just that one enabled source; `--source-dir <DIR>` (requires `<adapter>`) is a one-off path override that bypasses `[sources.<adapter>]` and never writes config. Sync ingests only already-enabled `[sources.*]`; it never discovers, enables, or writes source state - that is the explicit job of `pond sources` and `pond init`. With no enabled sources it does nothing but name the fix.
...
- `pond storage [check|use|migrate|creds]` - the storage cluster. Bare `pond storage` shows the resolved destination, creds binding, and per-table sizes. `check <URL>` probes a destination end-to-end (parse, creds resolution, the conditional-put OCC primitive, write/read/delete) with a distinct exit code per failure class (formerly `pond config check`). `use <URL>` is switch-only: probe the destination end-to-end, then flip `[storage].path` - it copies nothing, so switching between distinct stores, or rolling back to a previous one, leaves every store's data untouched. `migrate --from <URL> --to <URL>` is the only data-copy path: canonical data between storages over the export+import path - idempotent union merge (`lance-deterministic-pk` + merge-insert), re-runnable, valid onto a populated destination, never deletes or modifies the source (formerly top-level `pond migrate`). Copy is therefore always an explicit step, separate from the pointer switch. `creds <add|list|delete>` manages the URL-scoped `[creds.*]` sets in config.toml: `add` captures a set (name defaulting to `default`, secret via a hidden prompt) and writes `[creds.<name>]`, `list` shows configured sets with secrets redacted, `delete` removes one. It is a CLI affordance over the 3.9 model - sets still bind to URLs by scope match, never by activation, so there is no "current" set.
