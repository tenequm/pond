...
- `pond init` - idempotent setup-and-repair wizard: storage destination (with an end-to-end probe for remote URLs), adapter selection, MCP registration, and an opt-in sync schedule, written to `config.toml` in a single pass at the end. Every section is flag-answerable for non-interactive use: the destination via the global `--storage-path`, the rest via `--adapters`, `--schedule`, `--skip-mcp`, `--yes`, and `--force` (start from built-in defaults, ignoring existing config). `--yes` alone never schedules.
- `pond sync` - run the import, embed, and update-indexes stages in order. `--only <stage>` runs exactly one stage; `--skip <stage>` omits one. Stages are `import`, `embed`, and `update-indexes`. `--force-embed` re-embeds rows whose `embedding_model` differs from the current model via conditional merge. A positional `<adapter>` syncs just that one enabled adapter; `--source-dir <DIR>` (requires `<adapter>`) is a one-off path override that bypasses `[adapters.<adapter>]` and never writes config. Sync ingests only already-enabled `[adapters.*]`; it never discovers, enables, or writes adapter state - that is the explicit job of `pond adapters` and `pond init`. With no enabled adapters it does nothing but name the fix.
...
- `pond migrate --from <URL> --to <URL> [--skip-indexes] [--verify-only]` - copy canonical data store to store over the export+import path: idempotent union merge (`lance-deterministic-pk` + merge-insert), re-runnable, valid onto a populated destination, never deletes or modifies the source. On completion it rebuilds the destination indexes (defer with `--skip-indexes`) and verifies every row landed by id-set comparison - exit 0 synced, exit 6 if the destination is missing source rows; `--verify-only` runs just that read-only check, copying nothing. Distinct from `pond sync`: sync re-reads adapters, and a rotated or deleted source can no longer supply its rows (`session-durable-copy`), so migrate is the only path that carries the durable corpus to a new backend. Copy is always an explicit step, separate from the `use` pointer switch.
