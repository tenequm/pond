...
The MCP transport exposes the read operations - `pond_search`, `pond_get`, and `pond_sql` - as tools, plus the resources. Ingest stays HTTP-and-CLI only. Why: MCP's role is read access for an agent; ingest is an operator action.
