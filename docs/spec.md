...
- `pond copy [--from <url|file>] [--to <url|file>] [--no-optimize] [--verify-only]` - move canonical data between stores, `.pond` archives, and the JSONL wire stream. Each endpoint defaults to the configured store; an explicit one is sniffed by strict suffix: `*.pond` is a compact restorable archive (clean index-free Lance datasets plus a manifest; embeddings remain data columns), `*.jsonl` or `-` (stdio) is the JSONL wire stream, anything else is a pond store URL. Store-to-store is an idempotent union merge (`lance-deterministic-pk` + merge-insert), re-runnable, valid onto a populated destination, never deletes or modifies the source; on completion it rebuilds the destination indexes (defer with `--no-optimize`) and verifies every row landed by id-set comparison - exit 0 synced, exit 6 if the destination is missing source rows; `--verify-only` runs just that read-only check (store-to-store only), copying nothing. Archive/JSONL endpoints export from or restore into a store; JSONL is export-only. Distinct from `pond sync`: sync re-reads adapters, and a rotated or deleted source can no longer supply its rows (`session-durable-copy`), so `copy` is the only path that carries the durable corpus to a new backend. It is always an explicit step, separate from the `use` pointer switch.
