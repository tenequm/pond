...
`preview` is the one derived column whose values depend on a rendering choice rather than on a stored field alone, so the renderer's version is stamped on the column's schema metadata. A store therefore records which renderer produced its previews, and a changed renderer is a re-derivation over that one column - never a re-ingest, and never a schema change (the column's name and type do not move).
