...
5. **`pond_sql`** (`POST /v1/x/sql`, unstable) - one read-only SQL query behind the same read-only gate as the MCP tool, answered as JSON rows that self-describe (column names in-band), row-capped and byte-budgeted with the cut signalled. It lives under `/v1/x/` because its results follow the storage schema, which is not a wire contract: the request is versioned, the result shape is not (7.2). Discovery is the `schema://pond-sql` resource text - an MCP resource, so HTTP-only consumers read it from the source. Tenant scoping (7.3) does not come free: a multi-namespace pond must first scope every table-provider path (plain tables and the ranked `fts()` provider), plan metadata (`EXPLAIN` exposes whole-table statistics), and auth routing - future work for this operation, not an existing compatibility.
