...
- `pond sync` - the additive ingest verb: import fresh sessions (embedding: Section 8.7), then fold the search indexes and run the `[maintenance]` compaction pass (amortizing the version-cleanup walk over several runs since it is round-trip-bound on object stores) so the lake is queryable on exit. A positional `<adapter>` syncs just that one enabled adapter; `--path <DIR>` (requires `<adapter>`) is a one-off path override that bypasses `[adapters.<adapter>]` and never writes config. Sync ingests only already-enabled `[adapters.*]`; it never discovers, enables, or writes adapter state - that is the explicit job of `pond adapters` and `pond init`. With no enabled adapters it does nothing but name the fix. An enabled adapter whose configured source path does not exist on this host fails that adapter - named per adapter in the sync summary (`failed_adapters` in the JSON document, omitted when empty, each entry carrying the stable machine-readable `reason: "source_missing"`) and in the dry-run verdicts (the adapter's own `error` row, with a null session count) - and the run continues, exit 0 (`session-movement-complete`: the other sources are still reachable, and a shared fleet config legitimately enables adapters for tools a given host lacks); explicitly naming the adapter (`pond sync <adapter>`, with or without `--path`) keeps the hard error, since the operator named that source deliberately, and a malformed config blob still fails the whole resolve. An adapter whose source exists but holds entries it cannot read (an unreadable directory, an undecodable file) still runs: the losses are counted and the summary names the adapter with the counts and the first error - `degraded_adapters` in the JSON document, omitted when empty - instead of leaving them on the live progress line alone. It carries only what an operator can act on: whole files that yielded no session (`skipped_files`), events lost mid-session to a read or decode failure (`unreadable_events`), and sessions the validator rejected wholesale (`dropped_sessions`). The validator's whole per-event drop population is deliberately excluded from the health claim, including routine dedupe-floor drops (`adapter-integrity-dedup`) and ordering or structural violations; those counts are reported in the JSON document as a `drop_reasons` histogram accumulated before the outcome whose keys are the stable `DROP_REASON_*` tokens. The histogram spans every validator rejection, per-event and session-level alike, so a session-level key also appears in that adapter's `dropped_sessions` - one rejection, two surfaces, by design. Contractually unavailable transcripts are reported separately as the benign run-level JSON count `skipped_unimportable`, omitted when zero. `pond sync --dry-run --format json` and `pond status` both carry the same `reason` token as `failed_adapters`, so one condition never reads as two different failures; a cause that cannot be named carries no token rather than a guessed one. A whole-config run whose every enabled source root is definitely absent short-circuits after the flock and before the store: it still names each adapter, exits 0, writes the last-sync record, and emits the ok document, whose `stored` counts are null because it never opened the store to measure them. Per host and store, sync is single-flight: a local flock in the state dir (never on the Lance store - cross-host writers stay pure OCC) makes a second sync wait, naming the holder; `--no-wait` skips instead (exit 0), which is what the scheduled run passes so ticks never queue. `--dry-run` prints the freshness gate's per-adapter verdict (sessions, fresh, pending) and writes no session data; opening a new destination creates an empty store there, and the command may build the local freshness cache used to make the preview accurate. `--format json` emits one machine-readable summary document on stdout for every outcome - ok, skipped, or error (progress stays on stderr). Failures before the run stages use the compact `outcome` and `error` shape; only post-lock stage failures promise the counters accumulated before failure. These summary documents evolve additively within a major version: new keys may appear and are omitted when empty, while an existing key never changes meaning or disappears; a count a run did not compute is null rather than guessed. Error documents and last-sync records carry counts committed before the failure, except that an adapter failing mid-flush still omits its own committed batches (#240). Every phase that can run long has a live face - the freshness-map build and the per-adapter import bar (with an ETA from the bar's recent-rate estimator, never a whole-run average an early fresh-skip burst would poison) - and off-TTY (cron, agents) the bar line is emitted as a plain heartbeat every ~30s, including through the commit phase. A run that ingested rows re-extends the local freshness cache before exiting, so `pond status` pending counts are as-of sync end. Each run finishes by writing a per-host last-sync record (outcome, deltas, duration) to the state dir for `pond status`.
