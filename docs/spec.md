...
Every request carries `protocol_version` (a positive integer; v1 is `1`) and an optional `namespace`. Every request is associated with a server-generated request id for log correlation. HTTP exposes it via the `X-Pond-Request-Id` response header; MCP correlation uses the JSON-RPC envelope `id`. Schema evolution within a major version is additive only; removing or retyping a field is a major version bump. The one carve-out is the `/v1/x/` HTTP prefix, the convention for experimental operations: they still carry `protocol_version`, but have no additive guarantee and may change or disappear within a major version (7.5). The precise wire schema is published as JSON Schema generated from the Rust types: this document specifies the contract, the generated schema is the exact artifact.
...
5. **`pond_sql`** (`POST /v1/x/sql`, unstable) - one read-only SQL query behind the same read-only gate as the MCP tool, rejected before any dataset opens, and answered as self-describing JSON rows, row-capped and byte-budgeted with the cut signalled. It lives under `/v1/x/` because its result columns follow the storage schema (documented in the `schema://pond-sql` resource), which is not a wire contract (7.2). Tenant scoping (7.3) is not yet provided for this operation.

Three resources - `schema://pond`, `schema://pond-sql`, and `stats://pond` - expose the search-field documentation, the SQL surface's table schemas, and dataset statistics. The read-only SQL query surface is the `pond_sql` MCP tool (7.7), the `pond sql` verb (7.8), and its one fenced HTTP exposure, operation 5.
