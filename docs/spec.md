...
8. [Search and embeddings](#8-search-and-embeddings) - single-arm retrieval (full-text, with optional semantic search) and the embedding seam.
...
pond ingests sessions from agentic clients - Claude Code, Codex, and others on the roadmap - into one canonical form, stores them in Lance, and serves search (full-text, with optional semantic search; one arm per query, Section 8.7) over them at message granularity. It ships as a single static binary that exposes two transports - an HTTP+JSON API and an MCP server - over one shared set of handlers, and runs in two deployments (Section 2.2).
...
Automatic compaction runs only Lance-planned tasks that pass pond's filter. Tasks above the deletion-materialization threshold always run - reclaiming tombstones is useful even when layout does not improve. Any other task must strictly reduce the fragment count, predicted against both caps the rewrite writer splits output on - the output byte budget and the row target - because the binding one decides how many fragments come back: the byte measure rounds UP, while the row measure is the writer's own output count - a FLOOR of at least one, since the writer spreads surviving rows evenly across that many files rather than stranding a sub-target tail - and the prediction counts LIVE rows, what the writer actually writes, while the per-fragment width test below deliberately measures physical rows; and when its total bytes exceed one output file's byte budget, every input fragment's observed row width must let an output reach the row target within half that budget (headroom because re-encoding can widen rows relative to the input estimate). A task must also be worth its write amplification, judged against the mass a rewrite merely copies by two measures: its largest fragment, and the sum of its fragments already holding half an output by whichever cap binds (merging those at best halves the fragment count for a full copy of their bytes, so that copy has to be earned). Whatever volume is left beside either measure must reach a fixed fraction of it, except that a task whose volume is entirely such settled fragments has no remainder to weigh and is judged by the largest-fragment measure alone. Missing file sizes fail closed and skip the optional rewrite. Why: Lance selects candidates by row count while the re-encoding writer splits output by bytes, so a task whose rows are too wide for the target emits outputs that are immediately re-selected - an unbounded rewrite loop (measured: a ~1 GB table rewrote ~30 GB across 31 syncs with zero net progress). Predicting the output count from bytes alone reopened that loop from the other side under Lance 11, whose fixed row cap - set below the byte budget for freeze headroom - made it re-emit one output per input while the byte prediction promised a shrink (measured: ~80 GiB/day rewritten for a 4.5 GiB table); Lance 12 spreads rows evenly instead, which is why the row measure is a floor rather than a ceiling, and the measure is kept because even spreading assumes near-uniform row width - a task mixing very wide and narrow rows can still emit a byte-capped under-target output. Judging amplification by total-minus-largest hid the same tasks a second way - several settled peers read as data worth merging in, when the only new data was a few MB of appends; deferring those appends until they pay for the copy is the size-tiered amortization that bounds lifetime rewrites per row. A task whose whole volume fits one output cannot strand sub-target outputs, so the width test applies only beyond that point. Every rewrite re-encodes; none binary-copies. Why: a binary copy keeps each input page, so per-sync appends survive as thousands of tiny pages, and Lance 12 reads every page's metadata for each projected column on every take (measured: 11,300 GETs for a one-row `sessions` take, 15 after re-encoding).
...
| Inverted (FTS) | `optimize_indices(append)`; full recreate when the stemmer changed | Incremental fold is safe. The stems an index holds are those of the stemmer that built it, while Lance stems query terms with whatever the binary links; when Lance swaps stemmers (rust-stemmers -> frostem in lance 11: `added` -> `ad` vs `add`, `internal` -> `intern` vs `internal`) whole-word FTS silently misses in both directions. Pond therefore stamps the `messages` manifest config (`pond.fts.stems`) with the stems of a canary word list under the building binary after every build or rebuild, and the indices phase recreates an index whose stamp differs from - or predates - the running binary's, before any fold could append mismatched stems. `pond status` reports the index as `stemmer outdated`, and the first FTS query of a process logs a warning while it is. |
...
Index maintenance skips indexes whose columns no write has touched; this is sound because Lance prunes index coverage only for indexes whose fields overlap a write's modified fields. Which of these indexes a given instance maintains at all follows the embedding switch (Section 8.7). Why operator-triggered: matching Lance's own design (`table.optimize()` is the canonical periodic-maintenance op) avoids per-write index rebuilds that would otherwise dominate write cost.
...
Branching exists only between sessions: a session itself is a linear log of messages with no per-message parent pointers. `parent_session_id` records that a session was spawned from, forked from, or continued from another - a sub-agent, a fork, a compaction successor - matching the lineage operations Section 5.2 enumerates (sub-agent spawn, `/compact`, resume, and fork); `parent_message_id` additionally records the cut-point in the parent, for a fork-with-cut-point. A rotation that starts a fresh context without carrying the predecessor's history forward is NOT lineage: nothing was spawned, forked, or continued, and recording one would assert a relationship the source does not state (`model-no-synthesis`). A plain spawn (a sub-agent) populates only `parent_session_id`. `parent_session_id` is a soft reference: pond does not require the parent to be present at ingest, since independent adapter runs land in any order. Because a message id is unique only within its session, `parent_message_id` identifies the cut-point only together with `parent_session_id`; it is never resolved on its own.
...
Restoring a session MUST also restore its child sessions: the sessions that name it in `parent_session_id`. Why: a restored artifact must stand on its own in the target client - a Claude Code session that called the Task tool, restored without its subagent transcripts, is a set of dangling references rather than a working session. `parent_session_id` records a spawn, a fork, or a continuation (Section 4). The spawn graph is one level deep, capped structurally by the agent model - a Claude Code subagent cannot spawn subagents, and Managed Agents enforces a delegation depth of one. Fork lineage carries no such cap. Ingest records every lineage edge its source states, at any depth (`model-lossless-projection`): the depth-one limit is a RESTORE limit, not a model or an ingest limit, and an adapter MUST NOT drop, flatten, or refuse an edge to stay inside it - doing so would make an edge depend on which sessions happened to be ingested first, which `adapter-integrity-additive-sync` forbids. OpenClaw emits multi-level lineage today: a compaction successor is itself compacted, and its DB store forks from forks. Restoring a session whose child is itself a parent MUST surface as a typed error naming that child, never a silent partial restore (`adapter-integrity-no-silent-drops`).
...
A `pond_ingest` event is one canonical object - a Session, a Message, or a Part - tagged with its kind. Within a session's substream the order is fixed (`adapter-integrity-event-ordering`, Section 6). `Session.source_agent` and `Session.project` are immutable after first write, since both are denormalized onto `messages`: a re-submitted session with a differing value for either keeps the STORED value, and its rows are written under the stored labels rather than the attempted ones. The disagreement is counted (`relabeled_sessions`) and logged with both values, never silently accepted and never a per-row error. Why not reject the substream: the immutable field is already protected by keeping the stored value, so rejecting on top of that only discards the session's new messages. An adapter that corrects how it derives either field - a new source of truth, a fixed parse - would otherwise strand every session it had already stored, turning an improvement into permanent data loss on exactly the hosts that need it, and would make `--verify` report a gap no re-sync can close. A stored label that a current adapter would no longer derive is stale attribution, not a cosmetic one: `project` is the scope every filter and grouping relies on (`model-project-non-empty`), and a `source_agent` subpath governs default search exclusion (Section 8.3), so a mislabelled derived session stays visible where a correctly labelled one would not. Correcting a stored label is a deliberate migration (`adapter-integrity-additive-sync`), which this specification does not yet define. `pond erase` is NOT that path: its resurrection denylist (`session-append-only-exception`, Section 5.4) blocks the very re-sync a relabel would need, so erasing a mislabelled session deletes it rather than repairing it. A dropped message, by contrast, is unrecoverable by any path.
...
The server's instructions and tool descriptions MUST state only the capabilities the serving instance offers, and every statement about default behaviour MUST come from the running binary, never from an installed file (skill, plugin text). Why: fleets upgrade one host at a time; a file that names a default goes stale the moment the binary's default changes, and a description that offers an arm the instance refuses costs a wasted turn per agent per attempt.
...
Where a verb takes `--format json`, it emits one machine-readable summary document on stdout for every outcome - ok, skipped, or error (progress stays on stderr). Summary documents evolve additively within a major version: new keys may appear and are omitted when empty, an existing key never changes meaning or disappears, and a count a run did not compute is null rather than guessed. The exact document shapes are deliberately not inventoried here: they are pinned by the serde types that emit them and by the integration tests.

- `pond init` - idempotent setup-and-repair wizard: storage destination (with an end-to-end probe for remote URLs), adapter selection, MCP registration (one consent also installs the bundled agent skill - the same bytes `pond skill` prints - into Claude Code's user skills dir, kept in sync on re-runs; a hand-edited copy is never overwritten without an explicit confirm), and an opt-in sync schedule, written to `config.toml` in a single pass at the end. The MCP section also chooses the topology a client reaches pond through, explicitly and re-runnably: `http` points the client at one resident `pond serve` (registering it per the `pond service` verb below, then `claude mcp add --transport http` against its loopback `/mcp` URL), `stdio` registers a `pond mcp` child per client process. Interactively it is a prompt defaulting to `http`; `--mcp-transport http|stdio` answers it, and is what a non-interactive run needs to change a host's topology - with no flag, a non-interactive re-run keeps whatever shape the host already has and a fresh one takes `stdio`, since a `--yes` run may be sandboxed and an OS service registration would outlive the sandbox. A re-run that changes the shape removes the old client entry, writes the new one, and names the switch. A resident-server registration that fails (no service manager to supervise it) downgrades that run to `stdio` rather than pointing a client at an endpoint nothing answers. Every section is flag-answerable for non-interactive use: the destination via the global `--storage-path`, the rest via `--adapters`, `--every`, `--mcp-transport`, `--skip-mcp`, `--yes`, and `--force` (start from built-in defaults, ignoring existing config). `--yes` alone never schedules, and never registers a resident server. Embeddings are deliberately not an init concern (Section 8.7). After the config write, an interactive init offers to run the first sync in the foreground (full progress UI) and registers the schedule only once that sync completes - a fresh systemd timer fires immediately on registration, and two concurrent syncs only slow each other down. An opted-in schedule survives Ctrl-C during that first sync: the interrupt path registers it on the way out, so "the next sync resumes where this one stopped" stays true unattended.
- `pond sync` - the additive ingest verb: import fresh sessions (embedding: Section 8.7), then fold the search indexes and run the `[maintenance]` compaction pass (amortizing the version-cleanup walk over several runs since it is round-trip-bound on object stores) so the lake is queryable on exit. A positional `<adapter>` syncs just that one enabled adapter; `--path <DIR>` (requires `<adapter>`) is a one-off path override that bypasses `[adapters.<adapter>]` and never writes config. Sync ingests only already-enabled `[adapters.*]`; it never discovers, enables, or writes adapter state - that is the explicit job of `pond adapters` and `pond init`. With no enabled adapters it does nothing but name the fix. Source trouble is per-adapter, never per-run: an enabled adapter whose configured source path does not exist on this host fails that adapter alone, and one whose source is readable but lossy (unreadable files or events, validator-rejected sessions) runs degraded with its losses counted; both verdicts are named per adapter, under stable machine-readable reason tokens shared across every reporting surface (sync summary, dry-run verdicts, `pond status`), and the run continues, exit 0 (`session-movement-complete`: the other sources are still reachable, and a shared fleet config legitimately enables adapters for tools a given host lacks). Explicitly naming the adapter (`pond sync <adapter>`, with or without `--path`) keeps the hard error, since the operator named that source deliberately; a malformed config blob still fails the whole resolve. Per host and store, sync is single-flight: a local flock in the state dir (never on the Lance store - cross-host writers stay pure OCC) makes a second sync wait, naming the holder; `--no-wait` skips instead (exit 0), which is what the scheduled run passes so ticks never queue. `--dry-run` prints the freshness gate's per-adapter verdict (sessions, fresh, pending) and writes no session data (opening a new destination creates an empty store there, and the command may build the local freshness cache that makes the preview accurate). Every phase that can run long has a live face - the freshness-map build and the per-adapter import bar (with an ETA from the bar's recent-rate estimator, never a whole-run average an early fresh-skip burst would poison) - and off-TTY (cron, agents) the bar line is emitted as a plain heartbeat every ~30s, including through the commit phase. A run that ingested rows re-extends the local freshness cache before exiting, so `pond status` pending counts are as-of sync end. A successful run atomically records its store-derived per-session freshness cursor in the per-host state dir, keyed by store, so an embedded serve sync can resume after restart when a concurrent rowmap build has not yet installed the resident oracle. Each run also writes a per-host last-sync record (outcome, deltas, duration) for `pond status`.
- `pond optimize` - the maintenance verb: run the embed stage over the un-embedded backlog (embedding: Section 8.7), then fold the search indexes (the index stage also runs the `[maintenance]` compaction / version-cleanup pass). `--only <stage>` runs exactly one stage; `--skip <stage>` omits one. Stages are `embed` and `index`. `--force-embed` re-embeds rows whose `embedding_model` differs from the current model via conditional merge. `pond sync` ingests, folds indexes, and compacts on every run (cleaning versions only periodically); `optimize` is the on-demand catch-up - backlog embed and model-swap re-embed - and cleans versions every run. A bare `optimize` also diagnoses the expensive conditions it leaves alone - a model swap, piled-up index delta segments, orphaned indexes, fragments still in the binary-copied tiny-page layout (read from data-file footers: one GET per healthy fragment, two per legacy one) - naming `--full` and each heal's cost; `pond status` reports the same diagnosis read-only. `--full` heals them all idempotently, and only this explicitly typed verb does: pond never self-initiates a large rewrite, so `pond sync` only reports the manifest-level findings, at no extra request.
...
- `pond search [--explain]` - search from the command line. `--mode vector|fts` picks the retrieval arm (arm availability: Section 8.7) and `--sort-by relevance|recency` the order. `--explain` returns Lance's `analyze_plan` output instead of results.
...
- `pond status` - row counts, dataset statistics, index health, and the sync-schedule state, plus this host's relationship to the store: what each enabled adapter sees on local disk, how many sessions are pending sync (classified against the locally cached freshness map - never a remote scan; unknown until the host's first sync), the last sync's outcome (including a surfaced FAILURE from a scheduled run), and the estimated next scheduled run. `--hosts` adds the fleet view of a shared store - sessions and latest activity per ingest-host stamp. It prints the cheap storage summary first, then completes the longer checks.
- `pond serve --transport http|stdio` - run HTTP by default, or MCP over stdio. The `/mcp` route validates the `Host` header against an allowlist (the MCP spec's DNS-rebinding defence) that is loopback-only by default, so a server reached by any other name answers MCP with 403 until that name is listed with `--allowed-host <name>` (repeatable; env `POND_ALLOWED_HOSTS`, comma-separated); the `/v1/*` routes are not gated. `--with-sync` folds the periodic sync into the serving process (interval `--sync-every <minutes>`, default 5), so one process serves the read tools and keeps the store fresh in one warm process instead of a separate `pond sync` child cold-starting its own. This is a process-topology consolidation only - "no daemon beyond `pond serve`" stays literally true and it does not activate live-write (Section 9.4): the in-serve loop is the same interval sync, takes the same per-host lock with `--no-wait` (a cycle that lands while another sync holds it skips cleanly), writes the same last-sync breadcrumb, and routes all its output to stderr/tracing so stdout stays reserved for the transport. A failed cycle is logged and never takes the serving half down. `--bootstrap <adapter>` is an operator-opt-in, init-equivalent step at serve startup for supervised deploys (a plugin-managed sidecar being the motivating case): only when no `[adapters.*]` entries exist at all does it discover and enable the named adapter - a disabled entry counts as configured and is never touched - and it completes before the sync loop spawns, so the sync-never-enables rule above is preserved verbatim. Discovery failure downgrades to a logged warning naming `pond init`; serve still starts, and an empty store answers with an empty result and its scope count. Keeping this process resident on a personal host is `pond service` below, and is the registration `pond init`'s MCP section defaults to.
- `pond mcp` - alias for `pond serve --transport stdio`.
- `pond service start|stop|status|logs` - keep one `pond serve --transport http --with-sync` resident, registered with the same OS service managers `pond schedule` uses (launchd on macOS, a systemd user service on Linux, a logon-triggered Task Scheduler task running `pondw.exe` on Windows, each supervising the process: `KeepAlive`, `Restart=always`, `RestartOnFailure`). This is a process-topology verb, not a new daemon: the resident process is `pond serve` itself and nothing else is introduced, which is what keeps "no daemon beyond `pond serve`" literally true. Its whole value is that the prewarm, the rowmap build, and the search-index load happen once per host instead of once per MCP client process, and `--with-sync` folds the periodic sync into that same warm process under the same per-host lock semantics described above. Registration is loopback-only and refuses a non-loopback `--host`: the `/mcp` route's `Host` allowlist is loopback-only by default, so a loopback registration satisfies it on any port with nothing widened, and exposing the endpoint under another name stays an explicit `pond serve --allowed-host` deployment. It pins the resolved state dir and config file into the unit for the same reason the sync registration does, and `start` is idempotent - a re-run with the same flags is a no-op, a re-run with different ones (or after an upgrade) rewrites the unit and restarts the process. `status` reports the backend and the `/mcp` URL recovered from the live registration; on Linux the unit is a user service, so it stops at logout unless the host has lingering enabled.
- `pond schedule start|stop|status|logs` - manage the automatic `pond sync -q --no-wait` registration with the OS scheduler: launchd on macOS, systemd user timers (or a fenced crontab block) on Linux, Task Scheduler on Windows (the task Execs `pondw.exe`, a windowless launcher that runs the sync without flashing a console and exits with its status, so the task's `Last Result` is the sync's own; the task XML sets `DisallowStartIfOnBatteries=false`, `StopIfGoingOnBatteries=false`, and `StartWhenAvailable=true` for a launchd/systemd-equivalent posture - `Hidden` is set too but only affects the Task Scheduler UI listing). An unattended scheduled run can only ingest already-enabled adapters: `pond sync` never enables an adapter (that is `pond adapters` / `pond init`), so a background job can never grow the adapter set on its own; and it passes `--no-wait`, so a tick that lands while another sync holds the per-host lock skips cleanly instead of queueing. Registration pins the resolved `XDG_STATE_HOME` and the config file it was registered with into the job's environment, so a scheduled run sees the same configuration as a manual one: the scheduler daemon never sources shell rc files, and a shell-only override would otherwise split the lock and last-sync record between the scheduled and manual syncs, or leave a scheduled sync reading a different configuration than the user set. On Windows the same invariant is carried by argument rather than environment - a Task Scheduler `Exec` action has no environment block, so the registration-time state dir is baked into the task's `--state-dir` (and the sync log path into the launcher's `--log`, an action having no `StandardOutPath` equivalent either); registration warns when `XDG_STATE_HOME` is set interactively, naming the pinned path the task will not follow away from.
...
Two retrievers are available over the same corpus: a BM25 full-text retriever (`fts`, the default) over each message's indexed text, and a vector retriever (`vector`) over the message embeddings produced by the configured model. The caller picks one arm per query - there is no server-side fusion of the two. The vector arm ranks by cosine similarity with a gentle recency tiebreaker; the full-text arm ranks by raw BM25. Either arm scores by raw magnitude, never by rank position, so a hit's score does not shift with pool size or the caller's limit. Whether the `vector` arm is offered, and what happens when it is requested but unavailable, is governed by the embedding switch (Section 8.7). Results group to one summary per session, keyed on `session_root` by default - and on the individual message when the caller pinned one conversation via the session filter, where root keying would collapse the whole response to a single hit; a group's representative message is its highest-scoring one. Independently of the arm, a `recency` sort returns the in-scope messages strictly newest-first and labels the response as recency-sorted. Arm and plan attribution is operator-only and exposed via `pond search --explain`.
...
Embeddings are derived, not source data, but are produced inline at ingest (Section 8.7 governs when this runs): the ingest path embeds each message from its just-computed `search_text` and writes `vector` and `embedding_model` in the same append commit (`session-embed-from-canonical`, Section 5.5 - the derivation source stays canonical, only the timing is at ingest). `pond optimize`'s embed stage walks the leftover backlog - rows whose `vector` is null (embedding was off at ingest) or whose `embedding_model` is not the configured model (a model swap) - through the same seam. The seam is generic: a future consumer that wants vectors reuses it over its own table.

### 8.7 Opt-in

Embedding is opt-in by configuration (`[embeddings].enabled`, default off). With it off, no pond process loads a model, ingest writes no vectors, index maintenance ignores the vector index, and a `vector` request is refused. With it on, ingest embeds inline, `pond optimize --only embed` fills any backlog, and the `vector` arm is available on request; `fts` is the default arm either way. Instances sharing one store may differ: rows ingested by an instance with embedding off are embedded when an enabled instance next runs `pond optimize --only embed`.

### 8.8 Index lifecycle

Vector and full-text columns exist from table creation; turning embeddings on or off never needs a schema migration. Index maintenance follows `lance-index-maintenance` (Section 3.7), and for the vector index it follows the switch in Section 8.7: an instance with embedding off neither folds nor rebuilds that index and leaves an existing one untouched. Vector search uses brute-force flat scan below the activation threshold (currently 100,000 non-null vectors); above it the trained IVF_SQ takes over. Partitions = `num_rows // 4096`; `[search].nprobes` is operator-tunable for recall.
...
6. **Other.**
   ...
   11. Blob v2 part storage - swap legacy `LargeBinary + lance-encoding:blob=true` for the `lance.blob.v2` Struct extension (and bump `data_storage_version` V2.1 -> V2.2). Unblocked at the 10.0.0 pin: the compact blocker measured at v7.0.0-beta.16 (`compact_files` errored with "there were more fields in the schema than provided column indices / infos" through the generic struct branch) is gone - re-verified 2026-08-12 by compacting a 2.2 dataset whose blob.v2 payloads covered inline, packed-sidecar, dedicated-file, and null shapes, all round-tripping byte-exact; v8 shipped a dedicated blob-v2 compaction path with its own test suite. Same one-payload-per-row model; blob v2 adds the sidecar-file optimization for large external blobs and a separate `uri` Arrow sub-field (today the URL string is stored as UTF-8 bytes in the `data` column with the variant tag in `variant_data.data_kind`).
   ...
   13. Score floor (`min_score`): removed in 0.15 after zero observed use; re-add behind the vector arm only if a measured need appears.
