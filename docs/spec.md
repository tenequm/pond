...
- `pond share <session-id> [--to <url>] [--viewer html] [--yes] [--open]` - publish one session's full transcript to a public link, as a static export rather than a running service (pond's data already lives in object storage, so serving is just a public bucket). The session loads at full fidelity - `Store::get_session`, not the budget-paginated `pond_get` session view - so every part renders in full, not summarized. `HtmlRenderer` (v1's only viewer) emits a self-contained HTML page: inline CSS with a light/dark toggle, and image parts embedded as `data:` URIs (pond is lossless, unlike exporters that drop image content) up to `[share].max_inline_image_bytes` (default 2 MiB; over the cap falls back to a placeholder). No redaction is applied - the command requires `--yes` or an interactive confirm that says so plainly, and warns again in the printed output. `SharePublisher` (v1: `BucketPublisher`, writing to the `[share]` bucket over `object_store`) resolves credentials exactly like any other storage address (`creds-scope-match`) - `[creds.share]` scoped to the share bucket keeps publishing credentials separate from the private data-store credentials in `[creds.default]`. `--to <url>` publishes to an ad-hoc destination instead of `[share].bucket`, resolved through the same `?creds=`/scope/catch-all/ambient ladder as any other storage URL - no extra restriction. Unlike `[storage]`/`[creds.*]`, the `[share]` block itself has no `POND_*` env mirror (`storage-env-mirror` covers only `storage.path` and `creds.*`) - file-only, matching every other non-storage config section.
...
6. **Other.**
   ...
   13. `JsonRenderer` for `pond share` - structured JSON artifact for a hosted viewer app, behind the same `ShareRenderer` seam as `HtmlRenderer` (Section 7.8).
   14. Audio rendering in `pond share`'s `HtmlRenderer` - inline `<audio>` for voice-session parts; the part-type dispatch is reserved but unimplemented, blocked on a real voice-session fixture.
   15. Additional `SharePublisher`s - `GitHubPagesPublisher`, `SelfHostedPublisher`, a hosted managed-storage publisher - behind the same trait as `BucketPublisher`.
   16. Secret redaction in `pond share` - a scrub pass before publishing; the `redact()` seam is reserved but a no-op today.
   17. `pond share list` / `pond share rm` - management and expiry for previously published links.
