...
- Concepts are filed by **subject area**, one directory per area, listed
  below. A new area gets a directory once it holds two concepts; until then
  file under the closest existing one rather than creating a directory of one.
...
- [Per-sync watermarks must never come from Dataset::versions() on a remote store](storage/s3-sync-change-detection-oracle.md) -
...
- [Read path latency analysis and bottleneck attribution](storage/read-path-latency-findings.md) -
...
- [Embeddings are opt-in and FTS is the default search arm](search/embeddings-are-opt-in.md) -
...
- [Semantic vs BM25 retrieval efficacy and cost evaluation](search/semantic-vs-fts-usage-findings.md) -
...
- [Hybrid search score normalization and memory footprint tuning](search/embeddings-tuning-findings.md) -
...
- [Bilingual FTS tokenizer evaluation and word tokenizer adoption](search/tokenizer-experiment-findings.md) -
  ...
  gains.
- [Value-oriented agent session retrieval and evaluation methodology](search/session-retrieval-evaluation-findings.md) -
...
- [pond retrieval tools redesign - research and reasoning (2026-06-19)](search/retrieval-tools-redesign-research.md) -
...
- [What a bench-gate row does and does not prove](engineering/bench-gate-evidence-standards.md) -
...
- [CI compiler-cache config is per-job and never at the repo root](engineering/ci-compiler-cache-architecture.md) -
...
- [OpenClaw integration architecture and session storage findings](integrations/openclaw-integration-findings.md) -
...
- [Computing token usage and cost from a pond store](usage/token-usage-accounting.md) -
...
- [Fable 5 vs Fable 5.1: the queries behind the per-prompt comparison](usage/fable-5-vs-5-1-usage-queries.md) -
