...
- **Body** = the PR's `## Release note` section verbatim. It renders as indented prose under that bullet, so write it for pond users: markdown `- ` sub-bullets, the key measured numbers, ASCII only, no leading indentation (the template indents it).
- **Release lead paragraph + `**Upgrading:**` block** go in the headline commit's body after a lone `[release-note]` line; the template hoists everything after that marker to the top of the version entry. One per release - if two commits carry the marker, both sections are hoisted in commit order.
- **Merge** with `gh pr merge <N> --squash -t "<title>" -F <body-file>` - the body file being the PR's Release note section. Never let GitHub default the squash body to the intermediate-commit list.
