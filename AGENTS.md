...
`flake.nix` is the single source of truth for the dev toolchain - rust (read from `rust-toolchain.toml`), zig, cargo-zigbuild, rcodesign, the macOS SDK stubs, moon, protoc, uv, node/npm, kache. `gh` is deliberately not in it: use the host's (CI runners ship their own), so the host's GitHub auth wiring is never shadowed. `nix eval --json .#lib.toolVersions` prints the pins, and the flake-check job asserts the table's text form matches what it evaluates to. The Linux CI legs enter this shell through `.github/actions/devshell`; the Windows legs are the exception, where `.github/actions/windows-bootstrap` still installs its own moon/protoc, so bumping either of those two is two edits until that action is retired.
