...
- Instrumented one incremental `pond sync` finalize on the real S3 store (`cargo bench --bench copy_bench -- --profile-optimize <dir> --dest-url <s3>`): the per-table `cleanup_old_versions` version-log walk costs **~9 s** and reclaims roughly one version, dominating the **~15 s** finalize (the incremental index fold is the other ~6 s). It is round-trip-bound, the same failure mode as the `versions()` storm - a 5-min cron sync paying it on every run is pure waste. Fix: `MaintenancePolicy.cleanup_interval` gates the walk to every Nth manifest version (`DEFAULT_SYNC_CLEANUP_INTERVAL = 16`); only `pond sync` opts in, so ~15/16 syncs skip cleanup (finalize floor ~15 s -> ~6 s, fold-only), while `pond optimize` / `pond copy` keep interval 1 and clean every run. The gate reads `dataset.version_id()` (a cheap in-memory manifest field, never the `versions()` list). Skipping only defers reclamation - the next due cleanup sweeps the accumulated backlog - so it is always safe. `copy_bench --profile-optimize` is the regression guard.
