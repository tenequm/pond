...
`flake.nix` is the single source of truth for the dev toolchain - rust (read from `rust-toolchain.toml`), zig, cargo-zigbuild, rcodesign, the macOS SDK stubs, moon, protoc, uv, node/npm, kache. `gh` is deliberately not in it: use the host's, so the host's GitHub auth wiring is never shadowed. In CI, pond-ci jobs get gh from the pond-runner image and GitHub-hosted runners ship their own. `nix eval --json .#lib.toolVersions` prints the pins, and the flake-check job asserts the table's text form matches what it evaluates to. The Linux CI legs enter this shell through `.github/actions/devshell`; the Windows legs are the exception, where `.github/actions/windows-bootstrap` still installs its own moon/protoc and pins kache through kache-action's `version`, so bumping any of those three is two edits until that action is retired. `node` and `npm` are the same story in `.moon/toolchains.yml`, until those go versionless. The flake-check parity step guards none of these copies.
...
- `.moon/toolchains.yml` declares `rust` versionless on purpose: `rust-toolchain.toml` is the pin, moon resolves rustc from PATH, and the declaration exists so the host triple lands in every task hash. `node` and `npm` still keep exact versions, so moon provisions its own node through proto even though the devshell ships one; going versionless is a separate, staged change. `/flake.lock` is a declared input of the Rust file groups, so a nixpkgs-toolchain bump (zig, the C compiler) invalidates them. `flake.nix` is not, except on `repo:build-dist`: a protoc bump, which edits only `flake.nix`, still moves no test or lint input.
...
- Flow: commits land on `main` -> release-plz opens a `chore: release vX.Y.Z` PR -> **merge that PR** -> CI's `publish-release` job tags `vX.Y.Z` and publishes crates.io + the GitHub release (sole binary host) + the Homebrew formula + the repo flake (`ops/nix/release.json`, read by `ops/nix/pond.nix`). The tail after release-plz lives in `ops/scripts/publish-release.sh` and is idempotent: a failed publish recovers with "Re-run failed jobs".
