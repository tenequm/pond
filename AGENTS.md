...
- Adding an adapter: follow the playbook in `skills/pond-add-adapter/SKILL.md` (the `/pond-add-adapter` skill) - spec doc + sandboxed self-captured fixture + adapter + conformance suite via the shared harness in `packages/pond/tests/integration/adapter/mod.rs`, all in one PR.
