...
- Confirm a probe actually reads your path before citing a row as evidence, and never bracket a small effect with a single row. Rows compare only within the same `store` digest and `write_corpus` tag. [What a row does and does not prove](docs/knowledge/engineering/bench-gate-evidence-standards.md).
