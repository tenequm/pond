...
- `POND_BIN=/path/to/pond moon run pond:bench-gate` snapshots a prebuilt binary (e.g. the released one) through the CLI probes; the cargo-bench fields land null since those compile HEAD. A full old-side row needs the gate run from the old commit's checkout.
