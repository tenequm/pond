...
**Read `docs/spec.md` in full before working on this repo** - it is the repo's bible;
everything here roots from it. **Read `CONTRIBUTING.md` in full before producing a PR.**

Durable project knowledge - decisions with rationale, findings with evidence - lives in
the OKF bundle at `docs/knowledge/`. Load the okf-project-knowledge-base skill before
reading or writing it. After substantial work, run an end-of-task capture review: capture
what passes the bundle's fences, or say plainly that nothing does.
...
- `packages/pond/benches/docs/results.md` is the measurement log behind the performance rules below - raw bench output with the host it was taken on. Read it before benchmarking, so a new number has something to be compared against, and append the run to it afterwards.
...
## Object-store access rules (S3 perf, each measured - evidence in the knowledge bundle)

Every rule below has one root cause: on an object store latency is round-trip-bound, so the fix is always to issue FEWER requests, never to widen a limit. Read the linked concept before re-litigating one - each carries the measurement that settled it.

- **Never derive a per-sync watermark from `versions()` on a remote store** - use a messages-based key (25-160x faster). [Evidence](docs/knowledge/findings/s3-sync-change-detection-oracle.md). Source-side change detection (mtime stat vs JSONL tail-peek) is always local filesystem work and is not backend-sensitive.
- **Absent rows MUST append, never merge-insert** (spec.md#session-durable-copy) - merge is 5.47x slower over S3. A shared write seam must expose append-for-absent as a first-class mode rather than collapsing everything into `merge_insert`; `write_bench --only append|merge` is the regression guard. [Evidence](docs/knowledge/findings/s3-append-vs-merge-write-path.md).
- **`cleanup_old_versions` stays off the per-sync hot path**, gated by `MaintenancePolicy.cleanup_interval` (`DEFAULT_SYNC_CLEANUP_INTERVAL = 16`); only `pond sync` opts in, `pond optimize` / `pond copy` clean every run. The gate reads `dataset.version_id()`, never the `versions()` list. `write_bench --profile-optimize` is the regression guard. [Evidence](docs/knowledge/findings/s3-sync-cleanup-amortization.md).
- **Never tune Lance's AIMD rate limiter** - no `LANCE_AIMD_*`, no equivalent storage option. Throttle errors mean pond is issuing too many requests, not that the ceiling is too low. [Evidence](docs/knowledge/decisions/object-store-request-rate-policy.md).
- **Guard `count_rows` inequalities with `IsNotNull`** - `And(IsNotNull("col"), Ne("col", val))`, never a bare `Ne` on a nullable column (>25 min vs 7.35 s). Count a narrow co-set column, never a wide one (`embedding_model`, never `vector`; co-set per spec.md#session-embed-from-canonical). [Evidence](docs/knowledge/findings/lance-count-rows-predicate-guard.md).

## Benchmarking storage-path changes: both sides, before the new binary writes

- A change that materially alters the read or write path (lance/DataFusion upgrades, index roster changes, storage-layer rewrites) must be benchmarked on BOTH sides before landing. `moon run pond:bench-gate` covers both in one run - read probes/benches against the configured store, write benches (copy suite, commit sweep, index build/fold) against ephemeral scratch stores beside it - and appends one row per run to `packages/pond/benches/docs/bench-gate-baseline.jsonl`.
- Run the old-side row BEFORE the new binary ever writes to the real store: writes mutate store state and the old-side READ baseline is unrecoverable afterwards. The write metrics carry no such hazard. `POND_BIN=/path/to/pond moon run pond:bench-gate` snapshots a prebuilt binary through the CLI probes, but the cargo-bench fields land null since those compile HEAD - a full old-side row needs the gate run from the old commit's checkout.
- Confirm a probe actually reads your path before citing a row as evidence, and never bracket a small effect with a single row. Rows compare only within the same `store` digest and `write_corpus` tag. [What a row does and does not prove](docs/knowledge/findings/bench-gate-evidence-standards.md).
...
- pond is a pre-1.0 release-plz-managed crate. release-plz derives the bump ONLY from `feat`/`fix` commit *types*, so a breaking marker earns a minor `0.X.0` bump **only when it rides a real-diff `feat!:` or `fix!:` commit**. A `!` or a `BREAKING CHANGE:` footer on any other type (`docs!:`, `chore!:`, `refactor!:`, ...) or on an empty (`--allow-empty`) commit is SILENTLY IGNORED for the bump. If the breaking code already merged under a non-breaking subject, add a NEW real-diff `feat!:`/`fix!:` commit to carry the marker - never force-push to reword history. [Why, and the v0.12.0 release this was learned on](docs/knowledge/findings/release-plz-bump-semantics.md).
- Git hooks are moon-managed: `vcs.hooks` + `vcs.sync: true` in `.moon/workspace.yml` auto-wire `core.hooksPath` -> `.moon/hooks` on any moon run; the pre-commit gate runs `moon run repo:secret-scan repo:check-changelog repo:check-knowledge` (tasks defined in the root `moon.yml`).
...
- **A squash that loses the body repairs itself.** The prose lives in the commit body, and a merge that submits an empty description field overrides the repo default and drops it - immutably, since git-cliff exposes `pr_title`/`pr_number`/`pr_labels` but no `pr_body` to fall back on (happened on #191). So the first `commit_preprocessor` runs `.github/scripts/release-note-from-pr.sh`: for a `(#N)` subject carrying no `## Release note`, it fetches the section from that PR and appends it before parsing. Changelog and GitHub release body both render from this, so both stay correct however the merge was done - and a note can be corrected after merge by editing the PR description. Every failure path (no `gh`, no auth, no network) passes the message through untouched, so local `release-plz update` still works.
