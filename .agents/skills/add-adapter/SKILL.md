...
A key is not always enough. An agent may gate every run on account sign-in rather than on a key, and a per-model API key will not satisfy that gate: grok-build refuses headless runs with "Not signed in" until an OAuth device-code login completes, which needs a human at a browser. Prove the capture can actually run before scripting the session list - one throwaway headless turn in the sandbox, with the credentials you intend to use.

### 1. Read the writer, not the reader

Clone the upstream agent locally and locate the code that WRITES the session files - serializers, transcript stores, migration code - not code that reads them back. The writer is the authority on what can appear in a file; readers routinely tolerate less than writers emit. Note the agent version or commit you read: it becomes the `Verified against` line of the spec doc.
...
Some upstreams have no history to trace: a filtered monorepo export squashed to one "Synced from monorepo" commit (xai-org/grok-build), a vendored drop. `--unshallow` cannot recover what was never pushed. Say so in the doc and derive the legacy shapes from the back-compat code instead - `#[serde(default)]` fields, `alias` attributes, format-version constants, and the migration paths that still read old shapes. What the current writer can no longer produce is still what its reader tolerates, and that is the list you need.
...
| 7 | Lineage | How forks/spawns/subagents appear and map to `parent_session_id`. A child does not always carry its own link: a grok-build subagent records its kind but no parent, because the only record is parent-side (`subagents/<child-id>/meta.json`), and claude-code and nanoclaw carry the same shape in their own sidecars. That forces three moves - build the child-to-parent map from one walk of the whole configured tree rather than per session (a child may sit in a different bucket than its parent); store the parent-side record verbatim in the child's own `options.source`, so canonical is self-contained and restore can re-emit it; and declare the latch it creates - a child first ingested from a partial tree never gains the link later, because additive sync never rewrites a stored session row (`adapter-integrity-additive-sync`). | `adapter-lineage-complete-restore` |
...
| 9 | Restore face | Native restore layout, or `restore_unsupported` with a reason naming the caller's alternative (oh-my-pi precedent). If restore emits any file outside the restored session's own directory - a parent-side lineage sidecar (row 7) is the case that forces it - declare the file and its consequence: restore refuses on an existing target, so restoring that session alone into a live client home collides on a path the caller never named. | `adapter-native-restore-lossless` |
| 10 | Freshness oracle | How "has this session changed?" is answered cheaply; whether the source can rewrite/truncate in place (if it can, a tail peek is wrong). Settle the rewrite question by measurement, never by trusting a third-party parser: capture the file, run every mutating operation the agent offers (rewind, compaction, resume, fork, import), and byte-compare - if the pre-operation file is still a prefix of the post-operation one, the writer is append-only and a tail peek is sound. deja-vu documents grok-build as truncate-and-regrow; the 1.0.x writer appends a `rewind_marker` instead, and only the prefix check settled it. On an unchanged re-sync the gate must fire visibly (`SkipReason::Fresh`, counted as `skipped_fresh`) for every session except the ones the suite declares as `resync_rereads` with the reason (a source that gives a session no usable watermark) - the harness rejects an adapter that merely re-ingests idempotently. | `adapter-integrity-additive-sync` |
...
- Title, then one line naming what the doc was checked against: `Verified against: <agent> <version or commit>.` The **date** of that check is deliberately NOT repeated in the body - it lives once, in the frontmatter `verified` event (`verified:` with one `- { by: <actor>, at: <ISO 8601 instant> }` entry per confirmation), which is the OKF-native home for it. One date in two places drifts; one date in one place cannot.
...
Scope split (spec 6.9): the spec doc owns format archaeology and the decision record - facts about the upstream, which do not rot when pond's code changes. The adapter code stays authoritative for extraction behavior. Maintenance is best-effort: drift is passively detected (unknown kinds still ingest losslessly; malformed input surfaces as typed errors), and the doc's frontmatter `verified` event plus its `Verified against` line are updated when the adapter is next touched, not on a schedule.
...
- The freshness peek has a read budget, and `jsonl.rs` can measure it: wrap the call as `peek_metering::measure(|| adapter.peek_watermark(&path))` and assert the bytes read stay within `min(<file length>, PEEK_TAIL_CAP)` for every fixture transcript (`grok_build.rs` and `letta_code.rs` both carry this test). Bound it by the file length as well as the cap - a fixture smaller than `PEEK_TAIL_CAP` satisfies a cap-only bound even when the peek reads the whole file twice, which makes the assertion vacuous exactly where it is cheapest to get right. The meter is thread-local, because a process-global counter would cross-count under parallel `cargo test`, so it is reachable from unit tests only. If you add test-only items to a shared module like `jsonl.rs`, put them BELOW its `mod tests`: the import-isolation guard treats the first `#[cfg(test)] mod` as the start of the test section and flags every production item after it.

### 6. Docs

- Commit the spec doc from Phase A under `docs/knowledge/adapters/`.
- Add the adapter's row to the README supported-harnesses table, taking the date from the spec doc's frontmatter `verified` event.
...
When the agent is installed on your own machine, close the last gap the fixture cannot reach. Run one real session in your real home, then `pond adapters list` - the brand must appear as `detected` at the real path, which is `probe_default` against a live install rather than the injected `Env` that `assert_probe_default` simulates - then `pond adapters enable <name>` and a `pond sync <name>` with NO `--path`, so the discovered root is what gets read. Repeat it natively on Windows when a box is available: that run is what proves the probe resolves `~\...` with backslash separators and that a real Windows cwd survives as `Session.project` through the path encoding, neither of which a fixture path exercises.
